...
//
// This was a VERBATIM clone of encounter-privacy's two-regex pair, with NO
...
// polarity for the rest of the file, and blinds every check below — the gate
// then reports PASS *because* it went blind, a false-GREEN on a privacy gate.
// It now delegates to evals/rust-scan.mjs, which lexes comments and string
...
// AND string payloads are blanked to spaces, with length/newlines/offsets
// preserved instead of text being deleted — so `parseTableFields`, which
// brace-walks from `attrEnd`, stays exactly aligned with the raw source.
// ---------------------------------------------------------------------------
