 ...
 * `GuestClaimReaperSchedule` must not match the type name `Identity`, and
 * `IdentityToken` must not either.
 * @param {string} hay Text to search.
 * @param {string} name Identifier.
 * @returns {boolean} True on a whole-identifier match.
 */
function containsIdent(hay, name) {
...
// copied VERBATIM from account-privacy.eval.mjs (per-file local copy is the
// repo convention; see the header).
//
...
/**
 * Split a call's / signature's argument text at depth-0 commas, keeping both the
 * stripped (compacted) and the RAW spelling of each argument.
 ...
 */
function splitArgs(strippedInner, rawInner) {
  const bounds = [];
  let depth = 0;
  let angle = 0;
  let last = 0;
