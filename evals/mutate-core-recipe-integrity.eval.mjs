...
//   2. `.cargo/mutants.toml` (AC-M8): EXACTLY the three blessed exclude_re
...
//      three exclusions (no examine_re/exclude_globs/examine_globs).
//
// All three blessed exclusions are equivalent BY CONSTRUCTION, permanently —
// there is no contingent entry and therefore no self-expiry machinery here.
// (Slice EG1 added a fourth, contingently-equivalent entry pinning
// content.rs:624:5 load_evolution_paths -> Ok(vec![]); it was equivalent only
// while content/evolution_paths/ was deliberately empty. Slice EG3 authored the
// real graph, so that mutant is killable again and the entry was RETIRED in all
// three places its own self-expiry note named: the toml entry, the canary test,
// and this eval's pin. The list is back to three.)
//
...
// The three blessed exclusions. `key` is the mutated function name — the three
...
//
// The scan is quote-aware rather than a naive first-`]` search: an entry may
// legitimately contain literal bracket characters (e.g. an escaped `vec![]`
// token), which would truncate the body mid-entry. No blessed entry contains an
// escaped quote, so a plain in-string toggle is sufficient. Anything unquoted
// between entries other than whitespace and commas is rejected rather than
// ignored.
// ---------------------------------------------------------------------------
...
//   EXACTLY THREE exclude_re entries, validated PER ENTRY (not as one
//   concatenated blob — a blob scan is satisfied by a wildcard entry sitting
//   beside a comment-like entry naming all three pins, which would disable the
...
//     • every entry must be one of the three, byte-identical modulo regex
...
export function mutantsTomlPinned(tomlText) {
  ...
  if (entries.length === 2) {
    return {
      ok: false,
      reason:
        'mutants.toml exclude_re has only TWO entries — the third blessed exclusion (trading/types.rs:56:9 is_active, fix-nightly-mutants) is missing; EXACTLY THREE entries are required',
    };
  }
  if (entries.length !== 3) {
    return {
      ok: false,
      reason: `mutants.toml exclude_re must have EXACTLY THREE entries (found ${entries.length} entries / ${entries.length * 2} quotes) — the list is pinned to exactly three blessed exclusions (toward_home + apply_entry_ability + is_active)`,
    };
  }
  ...
  for (const entry of entries) {
    ...
    if (hits.length === 0) {
      return {
        ok: false,
        reason: `mutants.toml exclude_re carries an UNRECOGNISED entry \`${entry.slice(0, 80)}\` — every entry must be one of the three blessed exclusions (toward_home / apply_entry_ability / is_active). An unrecognised or wildcard pattern such as \`.*\` matches every mutant in game-core and silently disables the whole zero-tolerance gate (ADR-0050)`,
      };
    }
    if (hits.length > 1) {
      return {
    ...
    if (matched[hits[0]] !== null) {
      return {
        ok: false,
        reason: `mutants.toml exclude_re names the ${BLESSED_EXCLUSIONS[hits[0]].key} exclusion TWICE — each of the three blessed exclusions must appear exactly once (a duplicate hides a missing one behind the three-entry count)`,
      };
    }
...
// ---------------------------------------------------------------------------
// Canonical positive-control fixtures — the THREE blessed exclusions (EG1's
// fourth, contingently-equivalent entry was retired by slice EG3). The
// installed recipe + toml and these fixtures must be byte-consistent; if a
// later slice changes the wrapper or the toml, these controls document the
// contract the change must still satisfy.
// ---------------------------------------------------------------------------
...
#
# 4. RETIRED by slice EG3 (2026-08-05). The fourth entry
#    (content.rs:624:5 load_evolution_paths -> Ok(vec![])) was CONTINGENTLY
#    equivalent — only while content/evolution_paths/ was deliberately empty.
#    EG3 authored the real ten-edge graph, so the mutation is now killable and
#    is killed for real by \`eg3_load_evolution_paths_returns_the_authored_graph\`
#    (game-core/src/content.rs) plus the exact-shape pin T2 in
#    game-core/tests/eg3_evolution_graph.rs. Retired in all three places its own
#    self-expiry instructions named: this entry, the canary test, and the
#    four-entry pin in evals/mutate-core-recipe-integrity.eval.mjs. The list is
#    back to THREE permanent, equivalent-by-construction exclusions.
#
exclude_re = [
...
export default async function () {
  const name =
    'mutate-core-recipe-integrity (fix-nightly / ADR-0088 + M14.5h: mutate-core wrapper + .cargo/mutants.toml pinned to three blessed exclusions)'; // name kept for log-stability
  ...
  // (unbounded growth: only three blessed exclusions are allowed). The fixture is
  // the three blessed entries PLUS a bogus fourth, so the tooth bites on the count
  ...
  {
    const r = mutantsTomlPinned(tomlOverGrown);
    if (r.ok) {
      return {
        name,
        pass: false,
        detail:
          'TEETH 8 (over-growth): mutantsTomlPinned accepted a mutants.toml with FOUR exclude_re entries — the exclusion list is pinned to EXACTLY THREE blessed entries; unbounded growth would silence real surviving mutants (the fourth 53:15 entry is NOT equivalent and must get a killing test, not exclusion)',
      };
    }
  }
    ...
    'examine_re = ["parse_number"]\nexclude_re = ["npc/rules\\\\.rs:61:15: replace > with >= in toward_home"]\n';
  {
    ...
    if (r.ok) {
      return {
        ...
        pass: false,
        detail:
          'TEETH 10 (examine_re): mutantsTomlPinned accepted a mutants.toml with an examine_re key — examine_re narrows which mutants run AT ALL, changing scope for every nightly. Only the three blessed exclude_re exclusions are permitted; any other scope-shaping key must be rejected',
      };
    }
  }
  ...
  // 2+3) → must be rejected. Three blessed entries are required; a single-entry
  // file misses the ability.rs and is_active exclusions.
  ...
  // TEETH 15 — mutants.toml with toward_home + ability.rs only (missing entry 3)
  // → must be rejected. All three blessed entries are required; two is
  // insufficient.
    ...
    'exclude_re = ["npc/rules\\\\.rs:61:15: replace > with >= in toward_home", "ability\\\\.rs:170:60: replace < with <= in apply_entry_ability"]\n';
  {
    ...
    if (r.ok) {
      return {
        ...
        pass: false,
        detail:
          'TEETH 15 (missing entry 3): mutantsTomlPinned accepted a two-entry mutants.toml that is missing the third blessed exclusion (trading/types.rs:56:9 is_active, fix-nightly-mutants) — three entries are required',
      };
    }
  }

  // TEETH 16 (WILDCARD BYPASS) — three entries, one of them `.*` → must be
  // rejected. `.*` matches EVERY mutant in game-core, so it single-handedly
  // disables the zero-tolerance gate; the sibling entry here names all three
  ...
  const tomlWildcardBypass = `exclude_re = [".*", "npc/rules.rs:61:15 toward_home replace > with >= ability.rs:170:60 apply_entry_ability replace < with <= trading/types.rs:56:9 is_active with true", ""]\n`;
  {
    ...
    if (r.ok) {
      return {
        ...
        pass: false,
        detail:
          'TEETH 16 (wildcard bypass): mutantsTomlPinned accepted an exclude_re array containing a `.*` entry — that one pattern matches every mutant in game-core and silently disables the entire zero-tolerance mutate-core gate (ADR-0050) while the file still LOOKS like three blessed exclusions. Every entry must individually be one of the three blessed exclusions',
      };
    }
  }

  // TEETH 16b (DUPLICATE) — three entries where one blessed exclusion appears
  // TWICE and another is missing → must be rejected. A duplicate otherwise hides
  // a missing pin behind the three-entry count.
  const tomlDuplicateEntry =
    'exclude_re = ["npc/rules\\\\.rs:61:15: replace > with >= in toward_home", "npc/rules\\\\.rs:61:15: replace > with >= in toward_home", "ability\\\\.rs:170:60: replace < with <= in apply_entry_ability"]\n';
  {
    const r = mutantsTomlPinned(tomlDuplicateEntry);
    if (r.ok) {
      return {
        name,
        pass: false,
        detail:
          'TEETH 16b (duplicate entry): mutantsTomlPinned accepted an exclude_re array naming toward_home TWICE while the is_active exclusion is absent — a duplicate must not satisfy the three-entry count',
      };
    }
  }
  ...
  // POSITIVE CONTROL B — the canonical mutants.toml (verbatim, plan §T6) must PASS.
  {
    ...
    if (!r.ok) {
      return {
        name,
        pass: false,
        detail: `POSITIVE CONTROL B: mutantsTomlPinned REJECTED the canonical .cargo/mutants.toml (plan §T6): ${r.reason} — the predicate is too strict and would fail even with the blessed toml installed`,
      };
    }
  }
  ...
  // Real check 2: .cargo/mutants.toml pinned to the three blessed exclusions.
  if (!existsSync(tomlPath)) {
    return {
      name,
      pass: false,
      detail:
        '.cargo/mutants.toml is absent — it must carry the three line-pinned blessed exclusions per ADR-0088 + M14.5h + fix-nightly-mutants',
    };
  }
  ...
  return {
    ...
    pass: true,
    detail:
      'mutate-core recipe intact (whole-crate -p game-core, fail-closed missed.txt count-compare, no scope-narrowing / neuter / cap= knob) and .cargo/mutants.toml pins exactly the three blessed equivalent-mutant exclusions, per entry and byte-identical modulo regex backslashes: 61:15 toward_home (ADR-0088), 170:60 apply_entry_ability (M14.5h re-pinned), and 56:9 is_active (fix-nightly-mutants)',
  };
}
