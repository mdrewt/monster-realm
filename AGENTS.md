...
- **SpacetimeDB versions (ADR-0197, upgraded 2026-08-16).** CLI/host **2.8.1** · Rust module crate **2.8.1** · npm `spacetimedb` **2.6.0** (`^2.6.0`, lockfile-pinned — deliberately not yet bumped; 2.7.1 added SDK auto-reconnect that overlaps the hand-rolled reconnect path, ADR-0085/nh3/nh4, and needs live netcode verification first). **The crate version IS the product version — keep it equal to the CLI.** `.github/workflows/{ci,nightly}.yml` pin **2.8.1** in three places; `evals/observability-metrics-contract` B4 fails loudly whenever that pin and the CLI on PATH disagree — that tripwire is correct, do not weaken it. **Write 2.x module syntax** (`#[table(accessor = x)]`, `ctx.sender()`, `ctx.database_identity()`) — see `.claude/skills/spacetimedb-reducer`. The crate version **does** track the product version (crate 2.6.0/2.8.1 exist and ship with their CLI) — the older "crate ≠ product version" note in `Cargo.toml`/ADR-0180 was **wrong**; the module is simply a major behind. It runs fine: module wasm ABI is major `10` on both, and a 2.8.1 host accepts any same-major module. **Write 1.x module syntax** (`#[table(name = x)]`, `ctx.sender` as a *field*) until the `M-stdb-2x-module-sdk` migration lands. See `.claude/skills/spacetimedb-reducer`.
