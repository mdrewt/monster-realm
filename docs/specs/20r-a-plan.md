...
onChallenge/onAccept/onDecline/onCancel. onPvpAttack/onPvpSwap/onCare untouched.
`ARCHITECTURE.md` "Send gating" bullet: one sentence.

**battleView.ts** — `#pendingBattleId: bigint | null`; callback types `=> void | Promise<void>` (onCare docstring rationale);
...
`#applyPendingLock(vm.battleId)` last in `refresh()`; five PvE listeners route through `#dispatch`; `hide()` releases before `closeOverlayA11y`.

**raisingView.ts** — `onTrain` widened; `#pendingTrain` + `#trainButtons` (cleared beside `#careButtons.clear()`);
...
**evolutionView.ts** — `onEvolve` widened; `#pending: Set<bigint>` + `#choiceButtons: Map<bigint, HTMLButtonElement[]>`
...
**pvpView.ts** — four callback types widened; `#pending = false` (view-wide); `#dispatch(run)` +
...
| M-1 | main.wiring source scan | `function sendGuarded(` line contains `): Promise<void>`; comment-stripped regions after `onAttack:`…`onUseItem:`, `onTrain:`, `onEvolve:`, `onChallenge:`/`onAccept:`/`onDecline:`/`onCancel:` (pvp) each contain `return sendGuarded(` — anti-vacuity `indexOf ≥ 0` per anchor | void-returning callback → lock releases after one microtask (tsc-invisible) |
| M-2 | frozen short-circuit resolves | `sendGuarded` body region contains ≥2 `Promise.resolve()` | never-settling short-circuit |
...
- **R-20r-a-FOCUS** (a11y, real browser only): disabling the focused button drops focus to `<body>`; the trap is inert until the
  next refresh. Not new for PvE (every batch `replaceChildren`s), NEW for Train/Evolve/pvp. Out of scope; follow-up.
