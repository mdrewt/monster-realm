...
**Reconnect overlap wipe.** The app-level reconnect (ADR-0085) opens a new WS while the host may not yet have detected the old socket's close (up to the 30 s idle timeout below). When the old session's `on_disconnect` finally fires it deletes the presence rows underneath the new session; `client/src/net/connection.ts:665-669` deliberately treats the resulting `'already joined'` as benign, so the wipe stays invisible. One client lock depended on that wipe: `dismissPending` in `client/src/main.ts` self-healed through the conversation-row delete, so this slice also clears it in `onReconnect` (desync-guard H1) — the only client code change.
...
- `rb73_wiring_open_session_body_is_frozen_and_single_purpose` — exact body equality (red-team measured: without it, dropping the insert or recording `Identity::__dummy()` instead of `ctx.sender()` stayed CI-green while reopening the defect for every real socket) plus zero `Err(`/`accounts::`/`unwrap(`/`expect(`/`panic!(` to compensate for the `ctx.db.` before the guard.
...
- `rb73_wiring_on_disconnect_guard_precedes_and_body_is_frozen` — counts/ordering first (guard index < resolver index, each exactly once), then exact body equality, mirroring `m22s3b_resolver_body_order` (accounts_tests.rs). Kills M1 "guard deleted", M2 "guard inverted", M3 "moved below", M9 "own-row delete dropped", M12 "bare `return` body".
...
**Behaviour changes.** `player.online` (written `true` once in `join_game`, never `false`) now means "this identity has at least one live connection" rather than "this socket is live": `challenge_pvp`'s target-must-be-online guard (pvp.rs) therefore admits a target whose game tab is gone but who holds another connection (an overlapping reconnect, a second tab, or a token holder's socket — R-rb-73-TOKEN-WEDGE); an unanswered challenge still falls to its TTL reaper and an accepted-then-abandoned battle to the 60 s deadline. HTTP calls no longer force-resolve a live session; their own row lives only while the call executes (insert → execute → delete). A reconnecting WS session keeps presence alive even if the old session's close races the new open. Multi-tab (multiple concurrent WS sessions for the same identity) keeps presence alive until the last tab closes. A lone ephemeral connection (HTTP call, no live WS) is still last-out, so `evals/smoke-republish-on-disconnect-compat.eval.mjs`'s premise survives unchanged. Measured (X8, `memory/projects/gates/rb-73.x8-live-proof.md`): an authenticated `spacetime sql` or `spacetime call` is itself an HTTP-ephemeral connection, so an owner SQL read of `player_session` always sees ITS OWN row (a fresh connection id per read, `st_client` count 1 during the read) — an observation artifact, not a leak; a host restart drained everything except the reading query's row, confirming the launch replay.
...
- **R-rb-73-ABORT-PHANTOM** (reducer-security-auditor M-1) — if `on_disconnect`'s transaction aborts AFTER its own-row delete (host energy/datastore failure; no reachable panic in the four resolvers or the presence deletes today), the delete rolls back while the host still removes the `st_client` row, so no launch replay ever fires for it and the phantom row keeps `has_live_session` true for that identity — every later disconnect of that identity skips cleanup until the row is removed by hand. Durable and identity-scoped, hence strictly worse than the pre-fix single missed cleanup; not reachable by a client today. Mitigation path shared with TOKEN-WEDGE: additive `connected_at_ms` tail column + idle-session reaper.
