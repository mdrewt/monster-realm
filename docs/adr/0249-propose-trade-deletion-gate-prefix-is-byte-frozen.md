...
`#[cfg(debug_assertions)]` on the statement are KILLED by those two tests. ADR-0237:204-205 (shifted +2 by the reciprocal header lines ADR-0249 and ADR-0251 add to ADR-0237) therefore
...
with the reciprocal `**Amended-by:**` header line, because its Consequences (`:204-205`, shifted +2 by the reciprocal header lines ADR-0249 and ADR-0251 add to ADR-0237) state that rb-47
