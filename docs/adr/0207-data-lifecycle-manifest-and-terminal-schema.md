...
- **JS `REKEY_MANIFEST` object entries (measured red-on-arrival):** `checkRekeyCompleteness` (`evals/guest-claim-integrity.eval.mjs`) infers REKEY policy from `typeof policy === 'string'`; any object-valued entry reds `[G6/consumed]`. Supervisor's S2 launch brief explicitly restricts touches to `accounts.rs` manifest region only and defers JS-discriminator work to backlog (residual R-m22-s0-X1, target: backlog). **[RETIRED — the measurement was true when made, but rb-2 (PR #378, `ab35926`) replaced this typeof-string inference with an explicit `policy` discriminator parsed once by `classifyPolicy` under a new first clause `[G6/policy]` (ADR-0208 D1). Object entries are now the ONLY legal shape and a string-valued entry is what reds; `[G6/consumed]` is left named here because that is the clause the pre-rb-2 failure genuinely surfaced at; recorded by ADR-0223]**
...
**Why not the JS path:** Supervisor's S2 launch brief restrains touches to `accounts.rs` manifest region only and relegates JS-discriminator work to backlog (residual R-m22-s0-X1, unpromoted). Object entries in REKEY_MANIFEST are measured red-on-arrival ([G6/consumed] fails when `typeof policy !== 'string'`). **[RETIRED — true when measured, and reversed by rb-2: the discriminator now lives in a first clause `[G6/policy]` that requires an object (ADR-0208 D1). `[G6/consumed]` is deliberately left named because it is the clause that failed pre-rb-2 — `[G6/policy]` did not exist yet; recorded by ADR-0223]**
