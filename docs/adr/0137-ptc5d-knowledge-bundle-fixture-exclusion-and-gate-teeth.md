...
- **Negative / accepted:** the knowledge bundle's file set deliberately diverges from `battle-schema-snapshot`'s (documented in both comments + D1); future server-growth re-baselines edit one extra constant (D4). **Two prose bodies of amended-but-untouched ADRs are now stale** and are left as a **pre-existing ledger residual** outside ptc5d's touch-set (this slice edits only its reserved `0137` + the eval; the supervisor owns cross-ADR backlinks, per the ADR-0136 precedent): (a) ADR-0118 A2 still records the cap as 309 while the justfile holds 299 (the m17.5a re-measurement under ADR-0118 §4); (b) ADR-0050 A3 still says the ceiling is 340 and *"bumps inside the ceiling don't require an eval edit"* — now false under D4 (every cap move needs a lockstep const bump). Both should get an in-place `(amended to 299 by ADR-0137 D4)` note in the **ptc5f ledger-reconciliation** slice (plus the `Amended-by: 0137` backlink on ADR-0118 at ADR-index reconciliation).
