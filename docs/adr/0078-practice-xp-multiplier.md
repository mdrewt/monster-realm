...
1. **Unit tests in `game-core/src/combat/xp.rs`:**
   ...
   - Property: `practice_xp_reward(x, true).value() <= x.value()` for all x

2. **Structural source-guard in `server-module/src/battle_tests.rs`:**
   - Body MUST contain `practice_xp_reward(` — kills inline `/ 10`
   - Body MUST contain `WILD_IDENTITY` guard around the practice flag — kills hardcoded `is_practice`
...
| File | Change |
|------|--------|
| `game-core/src/combat/xp.rs` | Add `pub fn practice_xp_reward(base: Xp, is_practice: bool) -> Xp` |
| `game-core/src/combat/mod.rs` | Re-export `practice_xp_reward` |
| `game-core/src/lib.rs` | Re-export `practice_xp_reward` via `pub use combat::` |
| `server-module/src/battle.rs` | Hoist `is_practice` flag; call `practice_xp_reward` |
| `server-module/src/battle_tests.rs` | Structural proof-of-teeth tests |
| `evals/practice-xp.eval.mjs` | New eval with TEETH A + B |
