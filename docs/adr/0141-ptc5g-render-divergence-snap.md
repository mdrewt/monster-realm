...
- **Forward-looking constraint (the snap decision is memoryless-by-committed-target).** The check compares the new target only against the clock's *last-committed* target — it holds no cumulative-displacement memory across frames. This is correct given today's dual pacing: the server's `movement_tick` writes at most one own tile per `STEP_MS` and the client predictor drains at the same cadence, so a genuine >1-tile own correction always arrives as ONE reconcile → ONE net jump on ONE frame (caught), while the only atomic multi-tile own write (a zone warp, `movement.rs`) is reset-covered. The single-frame semantics is pinned by test T5 (a mid-slide continuation must still slide, discriminating `.target` from `positionAt(now)`). **The invariant a future maintainer must uphold:** any *new* mechanism that moves the player's own authoritative tile (knockback, conveyor/ice, teleport, a forced-move) must either deliver its full correction atomically in a single reconcile, OR route through `resetPredictionState()`/`resolver.reset()` — otherwise a correction fragmented across frames as a chain of ≤1-tile hops would slide instead of snap, silently reintroducing this slice's rubber-band. No such mechanism exists today (verified: the only own-tile writers are the paced `movement_tick` and the atomic, reset-covered warp).
