...
**Decision:** A11Y-29's token SSOT is a Rust const table in content.rs checked inside validate_content, not RON: any byte under game-core/content forces an out-of-touches CONTENT_VERSION bump. The required-key roster is serde-derived, never hand-kept.
...
(`game-core/src/monster/types.rs:44`), `StatusKind::matches`
(`game-core/src/combat/ability.rs:57`) and three `StatusEffect` matches in
...
- **The eight `Affinity` rows are deliberately unconsumed.** `client/src/ui/battleView.ts:274`
...
`statusBadge`'s default arm returned `''` (`client/src/ui/battleModel.ts:52-58`), which combines
with `... || null` (`:196`) and `if (card.status)` (`client/src/ui/battleView.ts:277`) to remove
the badge from the DOM entirely — a status becomes *invisible*, which is strictly worse than
colour-only. It is unreachable in a same-checkout build (`battleModel.test.ts:958-961` pins the
...
`client/src/ui/battleModel.test.ts:964` asserts `statusBadge(v.name).length > 0` over the
...
therefore exported as a named const and the m14.5d loop additionally asserts
`badge !== FALLBACK` and zero `console.warn` calls, restoring the bite.

### The palette (escalation §8.1, default (a))

The HP severity trio (`client/src/ui/battleView.ts:267`) was `#4a4 / #aa4 / #a44` — a red/green
...
colour independence remains the numeric HP text at `:274` and the bar width; the palette is
...
- **Residuals:** **R-m23-s8-TSDUP** (backlog) — `client/src/ui/battleModel.ts:39` `statusBadge`
  ...
  **R-m23-s8-TINT** (art ticket) — above. **R-m23-s8-TITLE** (S9/backlog) —
  `client/src/ui/battleView.ts:299` exposes skill affinity only through `btn.title`, a
