...
`privacy.rs` defines `pub(crate) fn purge_export_bundles(ctx: &ReducerContext, owner: Identity) -> usize` (cited by its declaration, not by a line number — ADR-0230's anchor form, extended here from gate anchors to ADR prose and scoped to this helper: the sibling rb-60/rb-61/rb-62 convention of re-dating a drifted number is deliberately not followed for it, because that number had drifted into this module's doc-comment header): it collects `export_bundle` rows via the `owner_identity` btree index (ADR-0126 idiom, mirroring `disarm_claim_reaper` in `accounts.rs`), deletes each by PK, and returns how many chunks it deleted — the `-> usize` arrived with rb-40 (ADR-0235). The body is frozen contract: `privacy_tests.rs` pins it byte-exactly in squashed form.
