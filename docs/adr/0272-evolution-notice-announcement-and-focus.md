...
3. **Focus returns only if the banner held it.** On the visible→hidden edge only, `render(null)` checks whether `document.activeElement` is inside the banner before hiding it (Chromium's blur fixup is async). After the DOM writes, it calls `returnFocus()` only in that case. A level check would re-fire the sink on every later store batch whenever the sink cannot move focus (the canvas is not mounted yet). `main.ts` passes `() => worldCanvasEl?.focus()`, the house landing place (ADR-0206).
...
- Focus retention is proven in happy-dom plus a mechanical never-`disabled` pin. A real-Chromium browser-tier check is not part of this slice (residual `CHROMIUM-FOCUS`). happy-dom never blurs a hidden or disabled control, so the verifier's mutant that reads focus-inside AFTER the `display:none` write survives every unit tier. Only a browser test can kill it.
