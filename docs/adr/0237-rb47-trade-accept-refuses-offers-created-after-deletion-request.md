...
**Extended-by:** ADR-0251 (rb-81 — the closed reducer roster reaches pvp.rs, battle.rs, economy.rs and ranking.rs; its R-rb-47-ROSTER-PVP residual bullet is discharged by a dated amendment below); ADR-0252 (rb-83 — cancel_account_deletion declines the incoming offers the stamp gate refused for the pre-cancel row; its R-rb-47-CANCELLAUNDER residual bullet is discharged by a dated amendment below)
...
## Amendment (2026-09-12, rb-83 — residual R-rb-47-CANCELLAUNDER closed)

The "Residual R-rb-47-CANCELLAUNDER (backlog; spec-change class). Flow B: D cancels the deletion (stamp cleared, status Active), accepts, C confirms, D requests again with a fresh stamp — equivalent to trading while Active; costs D a full new grace period" bullet above is discharged by ADR-0252, NOT by the deadline-binding mechanism that bullet implied (which contradicts AUTH-29 and PRV1-3, both of which require a cancel to clear `deletion_requested_at_ms`, and ADR-0195 D3). Instead, by a cancel-time sweep: `cancel_account_deletion` now declines, behind the AUTH-38 gate and before the status write, every active offer naming the caller as counterparty that `opened_commitment_is_refused` refuses for the pre-cancel row, so neither cancel→accept nor cancel→re-request→accept can consummate a post-request offer. The stamp still clears and the grace still restarts — AUTH-29/PRV1-3 untouched. D5's "no gate on confirm_trade" and the no-counterparty-oracle rule are unchanged. The trading-while-Active window on offers created AFTER the cancel is plain Active trading (by design), and Flow A (R-rb-47-PREDATING, wontfix) is unchanged. Four new residuals registered: R-rb-83-CHALLENGELAUNDER (pvp.rs `accept_challenge` analogue, MED), R-rb-83-CANCELORACLE (the cancel transaction now touches the public `trade_offer` table and becomes observable to non-callers, MED), R-rb-83-TRADEOFFER-RLS (world-readable `trade_offer`, pre-existing, LOW), R-rb-83-SPECPRV13 (harness spec PRV1-3 needs a sibling clause, LOW, supervisor-owned), R-rb-83-SCANORDER (accounts_tests.rs `stripped_for_scan` strips strings before comments, MED).
