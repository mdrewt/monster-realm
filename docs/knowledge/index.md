...
|------|------|-------|---------|
| schema-overview | Schema Overview | Schema Overview | 42-table SpacetimeDB schema for Monster Realm. 53 reducers. |
...
| tables/export_bundle_reaper_schedule | SpacetimeDB Table | export_bundle_reaper_schedule | SpacetimeDB table export_bundle_reaper_schedule. |
...
| reducers/on_connect | SpacetimeDB Reducer | on_connect | Lifecycle: record the live connection, then lazy-provision or touch an `account` |
| reducers/on_disconnect | SpacetimeDB Reducer | on_disconnect | Lifecycle: the disconnecting socket's row goes first; the trade / PvP / wild-bat |
...
| reducers/request_data_export | SpacetimeDB Reducer | request_data_export | Build the caller a fresh export: one chunk per exportable table (split at the ga |
