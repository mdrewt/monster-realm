...
- **render layer** (`client/src/render/`, M4b) — the renderer's functional core +
  ...
  `placeholderAssets` is the only one of the three that a unit test IMPORTS and EXECUTES:
  `placeholderAssets.test.ts` captures Pixi `Graphics` call arguments through a structural
  fake renderer — draw instructions, not rasterized pixels. (`world.ts` separately carries
  `world.test.ts`, a pure source scan that deliberately never imports it; `characterView.ts`
  has no test.) The suite exists because BOTH of the sprite's state cues must be readable
  WITHOUT colour (rb-57, ADR-0241): facing is the notch's position, and action is a
  per-action monochrome glyph of integer rects drawn in the notch's ink, which demotes
  `ACTION_TINT` to redundant reinforcement rather than the sole carrier. It owns no
...
structurally cannot see — and, since rb-17, also owns the CSS side of the same rule: no
`--custom-property` may be declared inside a `prefers-reduced-motion`-scoped at-rule
(`[A11Y-RM2e]`, the value a module would read back with `getComputedStyle().getPropertyValue()`),
no non-test `client/src` module may name any of fifteen CSS read-back or motion-event APIs
...
tier's claim was scoped to A11Y-27's STYLESHEET arm only while its RENDERER arm was unwired in
production; **17r-a wired it** (see that entry below), so the renderer-arm pair in
`client/e2e/reduced-motion.spec.ts` is now an ordinary green gate and its known-defect guard is
gone. **A11Y-32 and A11Y-33 are
...
via `window.__game()` rather than by vitest units, so vitest-v8 would score them ~0%
(DOM/Pixi/live-SDK). The one partial exception is `render/placeholderAssets.ts`, which
rb-57/ADR-0241 showed IS unit-runnable in node — `placeholderAssets.test.ts` imports it
and asserts its Pixi draw instructions through a fake renderer. It stays excluded anyway:
it is still a Pixi shell, its texture output is a rendering concern, and the exclusion set
is exact-set-guarded (below). The threshold was ratcheted 25 → **96** in
...
(the completed-deletion marker; the S3b cascade slice writes it — nothing writes `Some`
...
would demand the D6 self-disarm anti-pattern in the cascade). **m22-s3**
(ADR-0225) landed the PRV1-5 recheck skeleton (pure `reaper_should_run_cascade`
+ scheduler-keyed row re-read), the PRV1-4 terminal-cancel guard (distinct
`REJECT_ALREADY_DELETED` error) with a fail-closed twin in `delete_account`, and
the §4.7 gate predicate `should_reject_for_deletion` (`is_pending_deletion` now
delegates to it; S5's `guards.rs` wrapper must delegate too, never re-derive).
The PRV1-6 five-step cascade is **S3b** — blocked on G5 write isolation
(`accounts.rs` writes only its four owned tables, so each erase/anonymize step
needs a helper in the owning module, the `rekey_*` precedent). Until S3b a
...
`docs/adr/README.md` for the two-location navigation rules and the generated
`docs/adr/DIGEST.md` for the navigable catalog AND the derived next-free number:
the current range + one-line-per-ADR digest (the hard upper bound is deliberately
not repeated here; `DIGEST.md` is the sole enumeration, regenerated by
`just adr-digest` and drift-gated in `just ci` → `just eval` →
`evals/adr-digest.eval.mjs`, which runs the generator's `--check` mode). **ADR numbering note:** the harness
...
- **Shell (`net/connection.ts`):** app-level rebuild-with-backoff — one `scheduleRebuild()` timer handle, shared `handleDrop()` (store.reset → freeze → surface once → schedule), `wireTables` re-registers ALL table handlers per build, ONE `MicrotaskBatcher` across rebuilds, `joinGame` unconditional on apply with exact-match benign `already joined` catch, `pagehide` teardown + `pageshow(persisted)` bfcache inverse (RT-PH-01), getter-backed `conn` (never cache across await points). Hydration edge (17r-b, ADR-0130 residual d): the flush closure fires `opts.onHydrated()` once, on the first flush after each applied snapshot, after the view reconciles and before it notifies listeners, so every batch listener observes the store as already hydrated; `opts.onReconnect(identity)` carries this connection's identity from inside `onApplied` (after the stale-build guard), so an identity rotation on rebuild is visible to every identity-gated listener (residual e).
...
- **Error surface (`ui/errorRing.ts` + `errorOverlayModel.ts`/`errorOverlayView.ts`):** window `error`/`unhandledrejection` + augmented `reportError` + the rAF **frame catch** (17r-f: tagged `frame: `, collapsing consecutive identical messages so a 60 Hz thrower cannot evict ADR-0172 D1's reserved crash slots) funnel through `normalizeError` (cap `ERROR_MSG_MAX_LEN=512`, `ERROR_RING_CAP=64`) into a self-mounting (`#mr-error-overlay`), non-blocking (`pointer-events:none`, off the movement-suppression list), `textContent`-only overlay; re-entrancy-guarded.
...
(spec §4), so the sole production `resolve()` call in `main.ts` had to keep compiling unedited.
**17r-a** finally wired the live value at that call site (S5 never did); the field stays OPTIONAL
because tightening it to required would touch every `resolve()` fixture in `renderResolver.test.ts`. Own path: `reduceMotion` is
...
**m23-s4** complete, `M23-accessibility` S4 — **five constructed-shell views wired to S1's `openOverlayA11y`/`closeOverlayA11y`, pure client chrome — zero reducers, zero schema, zero predictor surface, no `main.ts` edits; S1's contract is factually WRONG and here CORRECTED.** Scope: `ui/{battleView,boxView,raisingView,evolutionView,claimView}.ts` plus the canvas world region in `render/world.ts`, all unedited from `origin/master` save the a11y wiring. S1's header and the `**m23-s1**` ARCHITECTURE block both claim the four `#app`-mounted views "share ONE root, so S4 must close-before-open or two capture traps stack" — but the code falsifies it: each view `document.createElement`s its OWN root and appends into the shared `#app` MOUNT (`battleView.ts:53/137`, `boxView.ts:32/86`, `raisingView.ts:49/89`, `evolutionView.ts:40/63`), yielding four distinct roots, four distinct `OverlayId`s and four distinct records. Close-before-open would have CLOSED an overlay still open. S4 deliberately did not implement it and pinned the correction with a cross-view test (`S4-CROSS-VIEW-DISTINCT-ROOTS` in `boxView.test.ts`) rather than amending the stale prose — the comment in `overlayA11y.ts` is flagged for whoever next opens that file. **The wiring shape**, and why each half is load-bearing: `wasVisible` read as the FIRST statement (a re-open tears down and re-schedules the deferred focus, dragging focus off whatever the player had Tabbed to — invisible to every attribute assertion since a re-open rewrites the same values); the close in `hide()` DELIBERATELY UNGUARDED (a guarded close reads correct while permanently leaking a live capture listener, a pending timer and an expiring return target; close-with-no-record is a documented no-op, so unguarded is the self-healing path — S3's red-team measured the guarded shape shipping 62/62 green); and the open as the LAST statement AFTER the display write (in a real browser `.focus()` on a `display:none` node is a silent no-op — S3 explicitly handed this residual to S4, and the new `OPEN-LAST` tooth captures `root.style.display` SYNCHRONOUSLY at invocation, because a post-hoc read of `mock.calls[0][1].style.display` was PoC'd to pass the wrong impl and the right one identically). `evolutionView` hoists the read above BOTH writes (it writes `display` before `#visible`, unlike the other three). **`claimView` has THREE doors and ONE nullity source.** `show()`, `hide()` and `render(vm)` all read the existing derived `visible` getter, never a shadow field (a `#lastRenderVisible` field never sees `hide()`, so after a dismiss the next `render(vm)` is not a field transition and the re-opened overlay would ship no role, no label, no focus and no trap while passing every single-cycle test). The REAL open edge fires inside `render()`, not `show()`: `main.ts`'s `openClaim()` runs `renderClaim()` → `show()` → `renderClaim()` and the phase has already moved by the first, so `render()` sees the transition one statement before `show()` is called. `render()`'s close arm IS guarded while `hide()`'s is not — the deliberate asymmetry. **Four missing DOM anchors created.** `OVERLAY_A11Y`'s `initialFocusSelector`s for these four ids are S0-frozen at `[data-testid="<x>-title"]` and did not exist; the DOM moves to the selector, never the reverse. Each `<h2>` gains `data-testid` + `tabindex="-1"` — `-1` not `0` because a heading with no tabindex makes the deferred `querySelector(...)?.focus()` a silent no-op, while `0` would pass `[A11Y-T5]` (which bans only `>0`) and add a permanent extra tab stop ahead of the overlay's real controls on four overlays. `claimView` gains nothing: its anchor `#claim-signin-btn` is already a real `<button>` and a natively focusable control must carry no `tabindex` at all. Heading `textContent` is byte-unchanged and asserted so — `client/e2e/recruit.spec.ts` resolves the box root off the literal `'Party & Box'` text. **`render/world.ts`** sets `role="application"`, `tabindex="0"` and `aria-label = t('a11y.world.region')` on `app.canvas` — never on `mount`/`#app`, which `main.ts` also passes as the parent of the four overlays, where an application role would swallow four dialogs and destroy their dialog semantics. The file is coverage-excluded, so the gate is a source scan that is BRACE-SCOPED to `init()`'s matched body and fails loud if the `mount.appendChild(app.canvas)` anchor moves — "the writes appear somewhere after the anchor" was PoC'd green against an implementation that left `init()` untouched and parked the three calls in a private method nothing calls. This is the FIRST `render/` → `ui/` import in the codebase (`ui/a11yCopy.ts` is a zero-import pure leaf; the reverse direction already exists; `net/` → `ui/` exists too). The deliberate fail-loud coupling: `t()` throws on a catalog miss, so a rename of that key would turn a missing label into a boot failure — CI-visible by design, and gated. **`touches-delta`, disclosed.** `ui/a11yCopy.ts` gains ONE key, `'a11y.world.region'`, closing exactly the gap the `**m23-s1**` block ESCALATED in writing ("`a11yCopy.ts` is in no slice's `touches:` after S0"): S0's own catalog header predicted the `a11y.world.*` namespace and scoped the set-equality gate to `a11y.overlay.*` on BOTH sides precisely so a later slice could add it without weakening an S0 gate, assigning its orphan check to the slice owning the consumer — here, `render/world.test.ts`. `ui/renameView.test.ts` gains a distinct new `it()` reusing S3's already-hardened stripper over the five S4 files (S3's own array and test left byte-identical, since its merged ledger CHECK pins them), rather than a second copy of a ~250-line stripper this repo has been bitten by twice. **KNOWN RESIDUAL, pinned not fixed.** `claimView` re-opens after a manual dismiss: `ClaimPhase` never returns to `'hidden'` and `main.ts`'s `KeyC` close calls `hide()` directly rather than through `applyClaim`, so a later reconnect-driven render arrives with `vm.visible === true` while the DOM reads hidden. That already re-showed the overlay before this slice; it now also announces and moves focus. The a11y layer is correct for the DOM state it observes — the defect is upstream in a model that cannot represent "dismissed". The fix needs `claimModel.ts` or `main.ts`, both out of scope (`main.ts` is S5's by spec §4), so S4 PINS the composed behaviour (`S4-claimView-REOPEN-AFTER-HIDE`) instead of hiding it. **Flagged upward for S10:** spec §5.5's vacuity-killer for `overlayA11yWiring.test.ts` requires the resolved `initialFocusSelector` element's tag to be in `{BUTTON, INPUT, SELECT, A, TEXTAREA}`, but S0 froze all four constructed-shell selectors onto `<h2 tabindex="-1">` anchors — S10 will fail on four ids unless that list widens. Gates: 39 new co-located tagged tests (tester ≠ implementer), 117 pre-existing assertions in the five extended spec files preserved with ZERO removed lines vs `origin/master`; acceptance ledger 9/9 met, 0 deferred, independently re-verified; red-team ran ~24 wrong implementations with ZERO green-and-wrong survivors; five proof-of-teeth bites (three by the orchestrator, two independently by the verifier) each reddened EXACTLY its own tooth with no collateral; full `just ci` exit 0 (94 client test files / 2684 tests, all evals PASS, `desync-guard` PASS). ADR next-free = 0206.

**m23-s5** complete, `M23-accessibility` S5 — **the sole `client/src/main.ts` touch: the scoped world-focus hotkey gate, ONE frame-loop announcement edge that finally makes S1's live region speak, the focus return to the world, and `#help-hint` promoted to a native `<button>`.** Scope: `client/src/main.ts` + the `#help-hint` element in `client/index.html`, plus their sibling test files and ADR-0206. Zero reducers, zero schema, zero predictor surface, zero view files. **The gate is SCOPED, never blanket** — spec §2.3 rejects `document.activeElement === canvasRoot` at the top of the listener on three citations (it would kill F8/F9, the menu intercept whose rows have no `tabindex`, and the fourteen-branch Escape ladder), so `worldHasFocus()` rides exactly the twelve `overlayVerdict(...)` **hotkey** guards — as the self-open disjunct `allow && (<self>?.visible || worldHasFocus())` (ADR-0206 **A1**, e2e-red fix cycle 1): a same-key press on an already-open overlay is a toggle-CLOSE and is never gated, because S3/S4's deferred focus makes "focus inside the overlay" the universal post-open state and the original whole-branch conjunct killed same-key close for every user (three merged e2e feature tests — movement-input:493, trade:97, the pvp:145 cascade — went red on PR #368 and encode that contract). Not KeyT (not a `canOpen`-derived guard, ADR-0164) and **not** the thirteenth `overlayVerdict('menuView')` at the delegated `[data-menu-launcher]` **click**: on a click `document.activeElement` IS the badge, so a gated front door would refuse the very interaction A11Y-23 exists to enable. Twelve near-identical conjuncts are deliberate duplication, not an oversight — `main.ts:378-380` states the rule in the repo's own words ("no single `!` can invert eleven gates at once"), and a shared `canOpenAndFocused()` helper would rebuild exactly that single point of inversion while hiding the gate from thirteen exact-equality body pins. **The `=== document.body` disjunct is load-bearing and pinned** (A11Y-35): a store-driven `render(null)` blurs a focused control back to `<body>`, and without it every hotkey is dead *forever* after any dialogue ends. Pre-initialisation is safe by construction — `worldCanvasEl` is `null` and `activeElement` is `<body>`, so the gate is open and behaviour is byte-identical to the fork. **ONE announcement edge, at the TOP of the rAF frame body**, after the `sessionGateBlocks()` early return and reusing the frame's single `performance.now()` (`liveRegion.ts` demands a monotonic clock and bans `Date.now()`), and *before* `predictor.drain` — the frame's `drain`/`resolve`/`render`/`nearestInteractable` calls are unguarded, so a tail-placed pump would let a recurring throw silence the region indefinitely AND freeze `lastA11ySnapshot`, collapsing several transitions on the recovering frame. The Escape ladder is **not touched at all**: `announcementsFor`'s own edge detection covers Escape, the store-driven `dialogueView.render(null)` close, the hotkey toggle-close and `refreshBattle`'s force-hide sweep alike, where a per-branch announcer would be blind to three of the four. Double-announcing is impossible by construction — the reducer emits only when `next.topOverlay !== null` and `main.ts`'s world branch fires only when `top === null`; a cross-file tripwire (`S5T-DISJOINT`) reds THIS slice's suite if a later slice closes S1's copy gap inside `announcements.ts` while the branch survives. **The focus return is NOT redundant with `closeOverlayA11y`, and that had to be written down.** Its restore order prefers `record.returnFocus` whenever still connected, and for a hotkey-opened overlay that is `document.body` — an `HTMLElement`, always connected — so the required `fallbackFocus` parameter is **unreachable on the common path** and handing it the canvas would change nothing. S5 therefore returns focus itself, guarded by `worldHasFocus()` so it never *steals*: open the menu by clicking the badge and `returnFocus` is the badge, the guard is false, and S5 leaves it alone (`S5T-FOCUS-NO-STEAL`). **`#help-hint` becomes `<button type="button">`** — Tab reachability and Enter/Space activation for free through the existing delegated click branch, so `main.ts` still never names the badge (`W-UX1-HINT-NO-JS-OWNER` survives, and it is a RAW-source pin: even a comment mentioning the id reds it, which it did once during implementation). But the terminal `if (e.code === 'Space') { jump(); e.preventDefault(); }` **cancelled the button's native activation**, which would have shipped A11Y-23 half-dead — Enter-only, invisible to every source scan since `main.ts` is coverage-excluded. Fixed by nesting the body in `!targetOwnsKey(e)`, reusing the ADR-0146 helper whose documented contract is exactly this case, and keeping the `if (e.code === 'Space') {` line byte-identical because it is a region fence. **Declared behaviour changes:** same-key toggle-close always works, even with focus inside the overlay — spec §8.4's accepted change ("`B` no longer toggles the Box closed") was **overturned** by its own overturn condition in fix cycle 1 (ADR-0206 A1); Space no longer jumps while a `<button>`/`<a>` has focus. **`touches-delta`, disclosed:** `main.wiring.test.ts` (unavoidable — `W-UXD3C-OPENGUARDS` pins all thirteen handler bodies by exact equality; twelve `expectedRaw` literals re-pinned, KeyT byte-identical), `indexShell.test.ts` (sibling of `index.html`; its own `:68-70` nominates S5 by name), the new `main.a11yFocus.test.ts`, and `docs/adr/DIGEST.md` (generated). **Three gate STRENGTHENINGS, each measured rather than asserted.** `W-ONE-CORNER-AFFORDANCE` goes tag- AND depth-agnostic (`body > div` → every inline-`position:fixed`, non-`inset:0` element at any depth): a second corner affordance shipped as `<a>` or as a nested `<button>` was invisible to the old selector and now bites. H4b's allow-list gains `background` outright and `border`/`padding` **with value clauses** (`border ∈ {0,none}`, `padding === 0`), so `border:50vw solid transparent` and `padding:0 50vw` still fail — and it closes a hole that predated this slice: `font` was allow-listed with no value constraint, and `font:900px/1 monospace` kept `width:max-content` and every allow-listed NAME while rendering the badge as a giant click-eating strip. **Accepted residual, documented not hidden:** `visibleIds(probes)[0]` is `OVERLAY_IDS` DECLARATION order, not z-order. `dialogueView` renders unconditionally on every store batch and force-hides only `menuView`, so a server-pushed conversation can become visible under an already-open overlay — `topOverlay` then does not transition (announcement missed) or names the wrong overlay while a `z-index:100` shell covers the screen. Both fixes are out of `touches:` (view/registry constraints, or changing the `A11ySnapshot` API S1 froze and S10 asserts against), so it is recorded for S6/S10 rather than worked around with a set-diff heuristic that would diverge from `announcements.ts`'s own documented contract. Gates: 15 new runtime-import tests over a REAL `main.ts` import (the `main.battle-reseed.test.ts` precedent, extended with `#app`, a canvas-appending `render/world` mock and a controllable rAF), 4 new source-scan teeth, 1 new markup tooth; twelve mutation bite-proofs measured by the orchestrator, all twelve BITE. Fix cycle 1 (ADR-0206 A1) adds 6× `S5T-GATE-SAMEKEY-CLOSE`, `S5T-GATE-REOPEN-AFTER-SAMEKEY-CLOSE`, `S5T-GATE-PRECEDENCE-DENY-WINS` and a second named exemption (census-pinned at exactly 12) in the `View?.visible ||` ceiling; full client suite 95 files / 2713+ tests. ADR-0206. ADR next-free = 0207.

**m23-s6** complete, `M23-accessibility` S6 — **`menuView` becomes an ARIA listbox, and the SIXTEENTH and
last `OverlayId` finally gets its a11y wiring.** Scope: `client/src/ui/menuView.ts` plus its sibling
spec; zero reducers, zero schema, zero predictor/renderer surface, no `main.ts` and no `index.html` edit.
**The wiring was S6's by construction, not scope creep:** S3's `touches:` listed ten views and S4's
listed five, and `menuView` appears in neither — `client/index.html:114-115` says so in the markup
("S6 owns menuView.ts; S2 may not add the role itself"), so it was the only one of the sixteen whose
`show()`/`hide()` never called `openOverlayA11y`/`closeOverlayA11y`. Without that wiring the slice's own
deliverable is inert: `aria-activedescendant` only speaks while the listbox itself holds DOM focus, and
the sole thing that focuses `#menu-rows` is `overlayA11y`'s deferred focus. The wiring copies the
`helpView.ts:42-58` shape exactly — `wasVisible` read BEFORE the display write so only the
hidden→visible EDGE opens, and the close in `hide()` **deliberately unguarded** so a record that ever
desynchronised from the DOM self-heals instead of leaking a live capture trap; `menuView` is a
`BATTLE_FORCE_HIDE` member driven from six `main.ts` call sites, which is what makes that asymmetry
load-bearing rather than stylistic. **The listbox is a CONSTRUCTOR-time contract** (`role="listbox"` +
`aria-labelledby` at the already-resolved `#menu-heading` element, never a second lookup and never a
literal): `replaceChildren` rebuilds the children, never the `<ul>`, and `aria-labelledby` sidesteps
§2.8's no-literals rule entirely because its value is an element id, not copy. Per row: `role="option"`,
an explicit `aria-selected` on EVERY row, `aria-disabled` **only when true** (`aria-disabled="false"` is
identical to absent), and **NO `tabindex` at all** — the ARIA APG activedescendant pattern puts tabindex
on the container only, and red-team MEASURED that a `tabindex="-1"` `<li>` is *mouse*-focusable, so one
click focuses the row, the next `replaceChildren` destroys that node, focus falls to `<body>`, and the
whole feature goes permanently silent. Spec §5.4's GOOD-fixture prose ("rows at `tabindex="-1"`") is
therefore **wrong and must not be encoded by S10**. **The option id is LEVEL-QUALIFIED** —
`menu-option-${vm.level}-${row.index}` — and that qualifier is not decoration: `buildMenuViewModel`
emits `index` = array position at BOTH levels, so without it "categories, Party selected" and "Party's
leaves, Monster Box selected" produce the SAME id, and descending a level leaves `aria-activedescendant`
at an UNCHANGED string. ATs announce an option on a CHANGE of that value, so the unqualified scheme was
silence on KeyM-then-Enter, the default path into the menu — found by red-team after the suite was
already 37/37 green, and fixed with the pointer built from the identical expression. The pointer is
written AFTER `replaceChildren` (a live IDREF, never a detached one) and `removeAttribute`d, never set
to `''`, when nothing is selected. **SPLIT KEYDOWN OWNERSHIP — the slice's one new pattern.** A third
delegated listener on `#menu-rows` (default bubble phase; capture fails two teeth) owns ONLY the
selection-movement inputs `up|down|left`, consuming them with `preventDefault` + `stopPropagation`
(never `stopImmediatePropagation` — a sibling listener on the same `<ul>` must still run) so `main.ts`'s
window listener does not step the menu a second time for one press. `enter` and `escape` — the only
inputs that can activate a leaf or dismiss the menu — deliberately keep bubbling, so activation and
dismissal retain `main.ts`'s ordered guard chain (`sessionGateBlocks()` FIRST, ADR-0182 D17/G20, then
the key-repeat gate, then the Escape ladder), and anything `menuKeyInput` does not recognise (KeyM above
all) falls through completely untouched. The subset is safe **mechanically, not by assertion**:
`menuStep` can only return `effect:{kind:'none'}` for those three, gate-pinned over the full
level × availability cross-product with a positive control, so a future `menuModel` edit that lets one
of them close or activate reds THIS slice's suite rather than silently routing a guarded action around
the session gate. Importing the pure `menuKeyInput` into the shell is SSOT forwarding, not an ADR-0014
breach — the alternative is a second `ArrowUp → up` switch inside a DOM file, and the subset filter is
an event-ROUTING decision, not a nav decision. **Declared residuals, out of `touches:`.** (i) A click on
the overlay backdrop/heading/back-hint focuses `<body>` (the overlay carries no tabindex and `#menu-rows`
is the only focusable inside it), which escapes the focus trap and silences the pointer — a milestone-wide
property of S3/S4's trap wiring, not introduced here, and unfixable in-file because A11Y-15 bans `.focus(`
in any `*View.ts`; needs `index.html` or `overlayA11y.ts`. (ii) `sessionGateBlocks()` can be true with the
menu visible, and the new listener moves the selection then — bounded to `renderMenu()` by the inertness
proof, and the pre-existing delegated *click* path already reaches full activation ungated. (iii) The
first close strips `index.html`'s STATIC `role="dialog"`/`aria-modal` until the next open — the house S3
pattern, no live consumer today. (iv) `main.a11yFocus.test.ts:313,:568`'s prose ("menuView … does not call
openOverlayA11y at all today") is now false and its exclusion of `menuView` from `SAMEKEY_OVERLAYS` rests
on that dead premise; behaviour is fine (`main.ts:1332` reads `visible` first) but untested — S10. (v) The
S5 handoff's `visibleIds(probes)[0]` z-order residual is NOT S6's; both fixes are outside `touches:`.
Gates: 19 new co-located tagged tests beside the 21 legacy blocks kept **byte-identical** (40 total,
57→185 assertions), tester ≠ implementer across two rounds; red-team ran 29 hand-written wrong
implementations and found **11 green-and-wrong survivors**, of which the six material ones were closed in
a second tester round (a `render()`-time re-open that yanked focus on every arrow key; a pointer built
from the array position; `dataset.menuIndex` from the array position; a hard-coded `'menu-heading'`
literal; an inline code→input map duplicating `menuKeyInput`; and a `visible` getter reading a private
boolean instead of the live DOM). The two source PRESENCE pins scan comment-STRIPPED text with a
...
**rb-12** (residual R-m23-s2-X6), ADR-0215. `evals/a11y-static-shell.eval.mjs` is the **sole owner** of `stripCssComments`; `client/src/indexShell.test.ts` deleted its duplicate and imports it. Consolidation was possible because `.ts` vitest tests CAN import `.mjs` evals (the reverse does not work — extensionless relative imports), which slice m23-s10 had concluded it was not. The duplicate was a live false-GREEN: the naive copy swallowed `display:none` on `.a{content:"/*"}.b{display:none}`, while the survivor is string-aware and fail-loud. Gates: a shared transition-total `CSS_STRIPPER_CORPUS` run by both tiers (eval teeth T10c/T10d/T10e; vitest RB12-G1..G7), with corpus payloads and cell names pinned independently in the `.ts` so corrupting the shared table cannot turn both green at once, and RB12-G7 pinning `parseCssRules` to the owner. 11 mutation bite-proofs, all RED their named tooth. Declared residuals: the repo goes from THREE `stripCssComments` variants to TWO — `evals/reduced-motion-hp-bar.eval.mjs:103` is NOT convergeable (it refuses comment delimiters inside strings, which `indexShell.test.ts`'s A6a BAD fixture 8 requires to parse), and `evals/reduced-motion-purity.eval.mjs:354` calls the hardened stripper without try/catch (inert today — loud-red never false-green if it ever fires). ADR next-free = 0216.

**rb-17** (residual R-m23-s10-X20), **no new ADR — no number was reserved for this slice; the supervisor should allocate 0218 if it wants the rulings below as a standalone record.** Closes two of the residual's three sub-parts. (a) `R-m23-s10-RMCSS` → `[A11Y-RM2e]`/`[A11Y-RM2f]`: S9's `@media (prefers-reduced-motion: reduce)` block (`client/src/styles.css:95-99`) is stable, so the ban can be written without having to be unwritten. It bans exactly ONE thing — a `--*` custom property declared under a motion-scoped at-rule — because honouring the preference declaratively is the intended design and a blanket `--` ban false-REDs S9's own `:root` colour tokens; a custom property is not a rendering effect, it is a JS-readable channel that re-creates the second preference reader while naming neither motion token. Three shapes are REFUSED rather than answered (CSS Nesting, which `parseCssStyleRules` renders invisible on BOTH axes; a top-level `@import`, matched case-INSENSITIVELY because CSS at-keywords are; and a symlink under the walk root). (b) `R-m23-s10-RMEXT` → `[A11Y-RM2g]`: MEASURED, the divergence ran the OPPOSITE way from the residual's text — `client/src` holds zero `.tsx`/`.js`/`.mjs`/`.cjs`, so the eval's five extensions bought nothing, while `listClientSourceFiles` skips `client/src/module_bindings` (65 vite-bundled, `main.ts`-imported files) and the shipped census did not. Reconciling DOWN would have been a 65-file loosening; rb-17 reconciles UP (92 → 157) and the `.ts` test imports the predicates (the rb-12/rb-15 direction). THREE new CSS strippers do NOT appear: the hardened `reduced-motion-hp-bar` one is imported under an alias for at-rule-ancestry work while `a11y-static-shell`'s stays the sole owner of the `index.html` pair per ADR-0215. Teeth 10 → 47; a second, independently-authored 65-fixture probe agrees. Twelve wrong implementations were written and measured; the two that survived the first draft (a case-sensitive `@import` scan and a read-back scan over the undifferentiated roster, the latter caught only by an incidental comment in an off-slice file) are the reason `hasTopLevelImport` folds case and `findCensusReadBacks` does its own narrowing. Declared residuals: **R-rb17-FOCUSHELPER** (the `[A11Y-15]` non-`*View.ts` helper gap — its recorded blocker "needs main.ts in touches" is WRONG, the scan reads files and never edits them; the real blocker is `evals/overlay-a11y-manifest.eval.mjs`, and ADR-0217 forbids a second oracle), **R-rb17-GEOM** (the geometric channel: a motion-scoped LAYOUT property read back with `offsetWidth`, unbannable without false-REDing the first legitimate layout feature), **R-rb17-WALKER3** (`reduced-motion-hp-bar.eval.mjs:1010` `listClientModules`, a fourth walker, out of `touches:`), **R-rb17-ADRCITE** (`docs/adr/0213-…:165` and `docs/adr/0215-…:126` cite line numbers in the file this slice grew; the ADR digest gate is header-only so CI cannot see the drift). ADR next-free = 0218.

**rb-15** (residual R-m23-s10-X18), **no new ADR** — the decision rides ADR-0215, whose body now carries an `Update (rb-15)` note reversing its "not closed by this slice" clause. `evals/a11y-static-shell.eval.mjs` is now the sole owner of the **whole** CSS oracle, not just `stripCssComments`: `parseCssRules`/`findIdSelectors`/`srOnlyIsAccessible` and their private helpers moved there, and `client/src/indexShell.test.ts` reaches them through its EXISTING namespace import (`client/src/indexShell.test.ts:89`) with **zero** new import lines — pinned biome 2.5.1 merges same-specifier imports, and a merged multi-line import takes RB12-G1's line-based count to 0, REDing it (measured). The operational win: `just a11y-e2e` (`justfile:348`) runs three evals plus eight named spec files and `indexShell.test.ts` is NOT one of them, so before rb-15 `[A11Y-06]`/`[A11Y-07]` were gated in the nightly a11y tier by a grep for `function findIdSelectors(` and nothing else. The eval now RUNS the oracle over two shared frozen tables (`ID_SELECTOR_FIXTURES` 14 rows, `SR_ONLY_FIXTURES` 21 rows, executed IN FULL by BOTH tiers) and over the real `client/src/styles.css`, with `@media`-nested and `.sr-only{display:none}` liveness probes and a RULE-count vacuity floor (a stylesheet gutted to its header comment passes `trim().length > 0` — measured). Teeth 24 → 81, with a new fail-loud evenness check (`evals/a11y-static-shell.eval.mjs`, the `TOOTH COUNT` clause — cited by name, not by line, because rb-15's own header rewrite exists precisely because line citations here went stale unnoticed) that previously lived ONLY in `justfile:365`, nightly. A6a/A7a are deleted and replaced by `RB15-G2` looping the shared tables — strictly stronger (`.includes(reason)` → exact sorted reasons SET); `RB12-G7` half 1 is deleted from the `.ts` and RE-CREATED in the `.mjs` as `T-REGION1/2/3`, because the bypass it closes survives the move verbatim one file over. Gates: `RB15-G1`/`RB15-G4` (shape ban + occurrence census + namespace integrity, all three required — measured: 7 shipping-plausible second oracles beat any two), `RB15-G2`/`RB15-G3` (shared tables + two-source roster pin), and 23 mutation bite-proofs, all 23 reddening their NAMED tooth. Declared residuals: **R-rb15-CASCADE** — the `[A11Y-07]` cascade/surface halves stay in the `.ts`, which is why `parseCssRules` is exported — and the `SUSPENSION_SPELLINGS` gap in `evals/overlay-a11y-manifest.eval.mjs` (`.skipIf(`/`.runIf(`/`.each([])`/`.for([])`), for which rb-15 added a LOCAL clause in the eval it owns and flagged the shared list as a follow-up. ADR next-free = 0217 (no new ADR: ADR-0215 already carries this design, amended in-body).

**rb-18** (residual R-m23-s10-X21), **no new ADR — and the number assigned to this slice, `217`, is ALREADY ALLOCATED AND MERGED** (`docs/adr/0217-a11y15-focus-oracle-single-owner.md`, slice rb-16, PR #392); the brief forbids picking a different one, so the rulings live here and the supervisor should allocate 0218 if it wants a standalone record. Substantively none is warranted either: no dependency and no production pattern is added — `Opened`/`OPENERS` are module-local to a spec file and exported to nothing (the rb-15/rb-17 precedent). ONE file changed, `client/src/ui/overlayA11yWiring.test.ts`, 84 → 116 tests. **The residual's premise was VERIFIED BY EXECUTION, not read.** Two 16-way mutation sweeps on `origin/master` before a line was written, deleting the single `if (!wasVisible) ` guard one view at a time: against each view's OWN spec, 16/16 mutants CAUGHT and every one by a repeat-open tooth (so the disposition is the residual's (b), not (c)); against the cross-view spec, 16/16 mutants SURVIVED, 84 tests green — the ADR-0010 RED-before state EXISTS, so this slice does not take the "no RED-before constructible" no-op exit the brief also permits. **One correction to the promoted scope:** the residual says all sixteen per-view specs ship a `-REPEAT-NO-REOPEN` tooth. FIFTEEN do; the sixteenth, `menuView`, ships the same substance under a different id (`client/src/ui/menuView.test.ts:1254`, `MV-A11Y-REOPEN-EDGE-01`) — true in substance, false in its literal naming, and the sweep confirms it bites. Renaming it is a declared NON-GOAL (churn in a file whose m23-s6 census region is cited from this document). **The fix.** `Opened` gains a REQUIRED `reopen: () => void` closing over the SAME view instance the opener already built — calling `OPENERS[id]()` a second time constructs a NEW view, and the five field-backed ids (`battleView`/`boxView`/`raisingView`/`evolutionView`/`pvpView`) read `this.#visible === false` on a fresh instance, so those five would legitimately re-open and the tooth would FALSE-RED on correct code. Thirteen ids re-open via `show()`; `dialogueView`/`questLogView`/`healView` via the `render(vm)` null→non-null edge; `claimView` via `show()`, matching `main.ts`. **TWO teeth, not one, and the second exists because red-team MEASURED a bypass.** `S10-WIRE-REPEAT-NO-REOPEN:<id>` (a repeat open is a NO-edge) and `S10-WIRE-REOPEN-AFTER-CLOSE:<id>` (after a close, a re-open IS a real edge: the helper fires again, the ARIA the close stripped is re-applied, and the deferred focus is re-armed). A per-instance one-shot latch substituted for the live-visibility read is INDISTINGUISHABLE from correct code to the first tooth, passes that view's own full spec, and permanently strands the overlay unlabelled, untrapped and unfocused after its first close — and TWELVE of the sixteen per-view specs carry no reopen-after-close coverage at all. Measured: the latch reds the mirror tooth 16/16 and leaves the repeat tooth GREEN, which is what proves the two teeth measure different defects rather than one doing both jobs. **A measured correction to this file's own header.** It called the `Readonly<Record<OverlayId, …>>` opener table's missing-id detection "a COMPILE error". `client/tsconfig.json` sets `"exclude": ["**/*.test.ts"]`, so `just client-typecheck` — and therefore `just ci` — never typechecks this spec, and vitest transpiles without checking: deleting an opener's `reopen` leaves `npx tsc --noEmit` at exit 0. The device is EDITOR-time only. The header now says so, and `S10-WIRE-TOTALITY` gained a RUNTIME opener-return-SHAPE assertion so the totality claim is carried by something CI actually runs. Two `afterAll` counters (`repeatChecked`/`reopenChecked` === 16) close the `it.skip` hole, which matters because `just ci` does not run the nightly `a11y-e2e` recipe whose `numPendingTests` clause would otherwise catch a quarantined tooth. **Bite-proofs, all re-executable:** 16/16 guard mutants red EXACTLY their own `S10-WIRE-REPEAT-NO-REOPEN:<id>` with zero collateral; 16/16 latch cheats red exactly the mirror tooth and leave the repeat tooth green; a missing `reopen` reds `S10-WIRE-TOTALITY` BY NAME; `it.skip` on one id reds the matching floor. Acceptance ledger 7/8 met, 1 deferred. **Declared residual — R-rb18-NIGHTLYFLOOR:** the nightly a11y decay ratchet's floor is NOT raised, though `justfile:345-347` says in words it should be. `just a11y-e2e` now reports `files=8 tests=213 floor=169` (181 before), so the gate is green and stays green (the check is `total < floor`), but the ratchet now carries 44 tests of slack — up to 44 a11y tests could later be deleted unnoticed, which is the exact decay it exists to catch. Raising it needs `justfile:348` AND the byte-verbatim copy of that whole recipe body pinned at `evals/ci-gate-wiring.eval.mjs:614`, which must move in the same commit or `just eval` reds; both are outside this slice's `touches:`. `justfile:324`'s "84 tests" prose is stale by the same two files (now 116). Two further residuals, disclosed by the reviewer lens and recorded rather than quietly fixed: **R-rb18-CLAIMDOOR** — the opener table drives `claimView` through `show()`, but `main.ts`'s `openClaim()` (`:454-462`) runs `applyClaim` -> `renderClaim()` FIRST, so `render()` has already flipped `wasVisible` and `show()`'s guard (`claimView.ts:118`) is a structural no-op in every current call path; the live door is `claimView.ts:107`, owned by `claimView.test.ts`'s `S4-claimView-THREE-DOORS`. Re-pointing the opener would also re-point five PRE-EXISTING per-id tests, so rb-18 discloses it in the opener's docblock instead. (`questLogView`/`healView` are closed here by `render(null)` where `main.ts:366-367` uses `hide()`; same disclosure, no behavioural difference since both derive `visible` live from the DOM.) **R-rb18-MAINCITE** — `main.ts:1574` is cited for the fresh-view-model-per-batch fact by `dialogueView.ts:16` and `dialogueView.test.ts:243`; it has drifted, the real site is the `onBatchApplied` block at `main.ts:1627-1641`. rb-18 corrected its own copy and flagged the other two rather than editing a production file outside `touches:`. ADR next-free = 0218.

**rb-36** (residual R-rb18-MAINCITE), **no new ADR — none was reserved for this slice, and none is warranted: no dependency and no production pattern is added** (the rb-15/rb-17/rb-18 precedent for a prose-only residual). **Comment/prose only — zero behaviour change, zero production logic touched.** rb-18 flagged that the citation `main.ts:1574` was carried for the fresh-view-model-per-batch fact and had drifted onto an unrelated `cureItems` `flatMap`; the real site is `dialogueView?.render(dialogueVm)` at `main.ts:1641`, inside the M12d `store.onBatchApplied` listener opened at `:1627`. **Seven citations were corrected across three files — five stale `:1574` and two stale `:362`, three more than the four this slice's brief named** — the same audit found `main.ts:362` (in fact the `boxView` force-hide thunk) cited twice in the same two files for the force-hide-table fact, whose deciding line is the `dialogueView` entry at `:365`. **The load-bearing decision is to stop citing bare line numbers for these facts at all.** Re-pointing `:1574` at `:1627-1641` reproduces the defect on a delay — a 2100-line `main.ts` drifts every slice, and nothing in `just ci` can see a stale comment citation, which is exactly why this one survived from m23-s3 to rb-36. Each citation now names a landmark and carries the number only as an explicitly dated hint. For the force-hide table the landmark is the UXD3C-HANDLES marker pair at `main.ts:348`/`:378`, which is **CI-PROTECTED**: `client/src/main.wiring.test.ts:4867` pins those marker literals against its main.ts reader, so the anchor cannot be silently deleted the way a line number silently slides. For the render site no marker pair exists near `:1625-1641` and adding one would mean editing `main.ts`, outside `touches:` — so the citation names the M12d listener, whose banner comment is unique in that file (verified, as is the handle table's own declaration line). **No anchor literal was planted** — every citation paraphrases its anchor instead of reproducing the exact source form, because **that trap bit this very record during the slice**: an earlier draft of this paragraph quoted the two bold paragraph markers `evals/rekey-contract-surface.eval.mjs` pins for uniqueness, planting a second occurrence of each and turning `just ci` red. A doc paragraph *about* an anchor can break that anchor's gate, so describe pinned markers, never quote them. **Per ADR-0224 no `evals/*.eval.mjs` was added, and no gate was manufactured for a prose fix**. The verifiable property — each anchor resolves uniquely in `main.ts`, and the fact it asserts is true of the resolved code — was checked once by the tester lens instead. Line `:1923` of this file carried the same drifted citation for the same fact and is corrected here. The rb-18 record at the end of this log is left untouched — it is a true historical statement about what rb-18 found and flagged, and this record is what closes it. **Three residuals, disclosed rather than quietly fixed, all outside `touches:` and all prose-only (no assertion, CI-neutral):** **R-rb36-WIRINGCITE** — `client/src/ui/overlayA11yWiring.test.ts:289-296` carries both the stale citation and a meta-citation saying the other two sites are flagged and untouched, which this slice's own edit makes false; **R-rb36-FOCUSCITE** — `client/src/main.a11yFocus.test.ts:782` carries the same drift; **R-rb36-ADR0206CITE** — `docs/adr/0206-world-focus-hotkey-gate-and-the-frame-loop-announcer.md:194` likewise, left alone because `docs/adr/**` is admitted to a slice only for its own reserved number and this slice has none. All three are the same defect class and should be swept together with the same anchor treatment. ADR next-free = 0234.

**rb-37** (residual R-rb18-CONCURRENT), **no new ADR — no number was reserved for this slice, and `ARCHITECTURE.md` records `ADR next-free = 0234`; minting an unreserved number races sibling merges** (the rb-15/rb-17/rb-18/rb-36 precedent). The supervisor may allocate 0234 if it wants a standalone record for the one novel precedent this slice sets: a shipped spec that spawns a CHILD test runner, the sibling of ADR-0205 D6's spec that spawns `tsc`. The defect was PRE-EXISTING and invisible to CI — `vite.config.ts` enables neither `--sequence.concurrent` nor `--sequence.shuffle`, so `just ci` never saw it — and was reproduced on `slice/rb-37` at `origin/master@318eb70` (76 failed/40 passed of 116) before a line was written, matching the rb-18 verifier's own figure exactly; that verifier also measured 43/41 on master itself, so the same root cause survives a different interleaving and this is a file-wide property no single slice introduced. The fix is one token: the file's single top-level `describe` gains a `.sequential` annotation, so all 116 tests (and any future addition) inherit isolation for free; measured 116/116 in all four sequence modes afterwards. True per-test isolation was rejected, not overlooked — four shared singletons (the per-file happy-dom `document`, the module-scope `beforeEach`/`afterEach`, the process-global mock spy, and the wiring module's private open-overlay set with no reset export) cannot be forked per test without injecting `document` into sixteen view classes, a production refactor outside this slice's one-file `touches:`; the residual's own text authorises the serial route with a documented reason, and a `vite.config.ts` setting cannot substitute because the CLI flag overrides config. Red-team produced a bypass reporting all-green counters while every `afterAll` coverage floor failed as a suite-level error and the run's own success flag was false and the child exited non-zero — so every acceptance gate asserts the child's exit status and the report's success flag, never the pass/fail counters alone. The proof-of-teeth is an ordinary sibling TS spec (no new `evals/*.eval.mjs`, per ADR-0224) that spawns child vitest runs and carries two anti-vacuity control arms: a generated, never-committed rendezvous fixture run both ways through the same flag constant and spawn helper the target arm uses, failing once without the flag and zero times with it, so the target arm cannot degrade to merely "the file passes vitest" or go silent if the flag is renamed. Two further red-team corrections were adopted: the shared test-title prefix count is asserted structurally rather than as a second hand-typed literal, and the generated control harness pins its concurrency limit explicitly because an explicitly-set limit of one silently neutralises the sequence flag on a two-test fixture (vitest's own shipped default is five, and is not the hazard). A `-t` filter against the new spec is unusable by design, since it marks non-matching arms pending and trips the spec's own completion floor. In-touches boy-scout work closes residual R-rb36-WIRINGCITE: the file's own citation comment, which had claimed two sibling drift sites were "flagged, not touched," was stale after rb-36 fixed both, and is retargeted onto a stable landmark rather than a line number, carrying no reproduced pinned literal. A second residual, R-rb37-TOOTHBODY, is declared on the artifact red-team's measurement that replacing one tooth's whole body with a tautology under an unchanged title leaves every arm green: 68 of the 116 teeth carry no completion floor, the condition is identical on master and orthogonal to concurrency, and adding more counters does not close it (a gutted body that keeps its increment passes the floor too) — the real oracle is mutation testing against the sixteen view classes, a gate of its own. RB37-G3's wording was narrowed to stop claiming otherwise. A third residual, R-rb37-SELFCOLLECT, is declared rather than fixed: nothing floors the client suite's file count, so deleting the new proof spec would leave `just ci` green with one fewer gate, and closing that needs a roster addition plus a matching byte-verbatim recipe pin outside this slice's `touches:`. Acceptance ledger: gates RB37-G1..G5. ADR next-free = 0234.

**17r-a** (residual R-rb-38-E1), **no new ADR — the supervisor reserved no number, and this file already records `ADR next-free = 0234` from two prior slices, so minting it here would race a sibling merge** (the rb-15/rb-17/rb-18/rb-36/rb-37 precedent). None is warranted on substance either: no dependency and no production pattern is added — the injected-flag design was decided at m23-s7 and this file plus the preference module's own header already pre-wrote the exact three lines, so the slice EXECUTES a recorded decision rather than making one. **The defect was that nothing executed it.** `motionPreferenceFromWindow()` had ZERO production callers — confirmed by the UNION of both code graphs plus a grep for dynamic dispatch, not by one graph — so `ResolveInput.reduceMotion` took its `false` default on every frame of the shipped client and BOTH reduced-motion render arms (the own-entity snap arm and the remote interpolation bypass) were dead code. m23-s7's module header names S5 as the wiring site, but s5 landed BEFORE s7 and nothing closed the contract afterward; the purity eval checks ownership only and structurally cannot see an unconsumed module. **The fix is three lines**: the preference is constructed ONCE at module scope beside the resolver it feeds, and `reduceMotion` is read per frame off its live getter at the render loop's single `resolve()` call. One construction plus a per-frame getter read IS a per-frame live read, because the getter closes over a value the change listener updates. **A hard constraint governs the comment as much as the code**: two independent RAW-text censuses (the m23-s7 source scan and the purity eval) permit the preference-reading tokens in exactly one non-test `client/src` module, comments included, so an explanatory comment naming the mechanism would have reded both gates; the shipped comment names the contract it closes instead. **Four teeth in a new happy-dom runtime spec**, not a source scan — the planner proposed a scan tooth and `/simplify` overturned it as forgeable, since presence pins cannot distinguish a wired call from a hollow binding. The tooth ids are deliberately PREFIX-FREE: red-team MEASURED that the originally-planned name for the fourth tooth contained the first tooth's id as a substring, which made the ledger's exactly-once census BOTH unsatisfiable for a correct implementation AND a false green when the first tooth was deleted outright, because the longer title still carried the substring. The gate runner now re-derives that prefix-free property at runtime, so a future rename reds there instead of silently hollowing the census. **Two teeth exist for wrong implementations nothing else catches**: a read-once-at-boot snapshot is behaviourally correct on a machine whose preference never changes and is killed ONLY by the live-change tooth, and a per-frame reconstruction is correct on every value assertion while leaking a media-query listener every frame, killed ONLY by the single-construction tooth. Both were confirmed as sole killers by mutation: seven mutants plus a control, each pinned to one named tooth, plus a harness mutant that stops the frame loop and reds all four with a loud armed-callback error so the suite cannot go vacuous. The spec is `describe.sequential` from the start, because red-team measured the modelled precedent failing 4 of 26 under concurrent sequencing — the same file-wide hazard rb-37 fixed elsewhere, inherited by copying the harness. **The slice also retired rb-38's known-defect alarm**, a declared scope extension outside the seeded `touches:` and required for merge: that alarm was built to flip RED the instant this wiring landed, it runs on the PR path rather than only nightly because the e2e script invokes every declared Playwright project, and its own failure message instructs the successor slice to delete the guard. Both of its test titles are kept byte-identical because the nightly recipe pins them by title and the whole recipe body is byte-pinned by an eval; renaming needs a justfile edit outside this slice. Its drifted `main.ts` line citations are retargeted onto landmarks per rb-36's doctrine, and no pinned literal is reproduced anywhere in this record, for the reason rb-36 records. **Follow-ups disclosed, not fixed, all outside `touches:`**: **R-17ra-JUSTPROSE** — the nightly recipe's prose header still tells readers the renderer arm is unimplemented, false as of this slice, comment-only and above the byte-pinned region; **R-17ra-OWNERHDR** — the preference module's cross-slice contract header still says it ships unconsumed and cites two line numbers that were already stale; **R-17ra-TITLE** — retiring the now-inaccurate defect label from the e2e title, which must move the title, the recipe pin and the eval region in one commit; **R-17ra-REQUIRED** — tightening `ResolveInput.reduceMotion` from optional to required, which touches every resolver fixture; **R-17ra-AMBIENT** — two e2e specs assert the own character DOES glide, which this wiring makes contingent on the default Playwright project's preference state rather than on nothing. The desync-guard lens MEASURED that this is a robustness nit and not a live flake: the pinned Playwright's context options default the preference to no-preference and EMULATE it rather than inheriting the host, so those specs are not host-sensitive. Pinning it with an explicit emulation call is still worth doing for the reason ADR-0219 D5 exists. **Three further residuals came from the ARTIFACT red-team, which hunted after the plan red-team and found a disjoint set — the recorded reason both passes are run.** **R-17ra-ADR0219** — ADR-0219 and the generated digest rows that copy its title and decision text still assert, in the present tense, that the renderer arm is unwired and unimplemented; that is the one live "asserts the old state" artifact this slice missed, and it is left alone because a slice is admitted to the ADR directory only for its own reserved number and this slice has none (the rb-36 precedent for the identical situation). It is prose-only and CI-neutral, since the digest gate is header-only. **R-17ra-PURITYSPLIT** — the purity eval's census is a RAW SUBSTRING scan for two literal tokens, so splitting either literal across a join boundary in a new module evades it: a decoy reader built that way and wired into `main.ts` in place of the owner was MEASURED passing both arms of gate B1 while the owner module became unconsumed dead code. Token-splitting is a pre-existing declared residual of that eval rather than something this slice introduced, and the compensating control is the mandatory desync-guard review, which ran and passed; the gate runner's own comment was corrected to stop claiming the arm proves single-ownership. **R-17ra-TOOTHBODY** — replacing a tooth's whole body with a tautology under an unchanged title still reports four of four, because the ledger censuses titles and not bodies; this is the same repo-wide limitation already on record as R-rb37-TOOTHBODY, whose stated oracle is mutation testing rather than more counters, and this slice's mutation evidence is the compensating control. Acceptance ledger: gate B1. ADR next-free = 0234.

**17r-b** (ADR-0130 residuals (d) and (e), disclosed by 16r-f and tracked nowhere until review 17), **no new ADR — the supervisor reserved no number, this file already records `ADR next-free = 0234`, and minting an unreserved number races a sibling merge** (the rb-15/rb-17/rb-18/rb-36/rb-37/17r-a precedent); ADR-0130 is amended in place instead, the 16r-f precedent, because both mechanisms were that ADR's own proposals. **Two defects.** The 16r-f reseed latch resolved on the FIRST defined battle read after a reconnect, so if the participant-scoped battle view hydrated across flushes with two or more rows, an older row observed first burned the latch and the surviving battle still emitted a spurious battleStart into the event ring and the F9 bundle (d). Separately, the reconnect callback carried no identity while the connection shell re-mints one on every build, so a rebuild that changes identity — reachable today through the anonymous-continue path after a session-expired terminal, or the nh4 token-rejection suppression — left every identity-gated listener in main.ts, the reseed listener included, deaf (e). **The fix is one widened callback and one new required one.** The reconnect callback now carries the new connection's identity and main.ts reassigns its module-local identity as that handler's first statement, so the connect ring event and every batch listener see it. A new required hydration-complete callback is armed inside the SDK's applied handler after the stale-build guard and fired by the batcher's flush closure on the first flush that follows — after the view reconciles, before the store notifies listeners, outside the live-connection guard so a parked build cannot leave it armed. main.ts keeps ONE boolean, set by that callback and reset unconditionally on every reconnect while the drop-time battle capture keeps its 16r-f guard; the latch returns while un-hydrated and otherwise resolves, and a post-hydration undefined read is definitive (no battle rows), so the resolved branch null-guards the row. **Reachability, recorded rather than glossed:** on spacetimedb npm 2.6.0 the SDK applies the cache, emits applied, then dispatches the row callbacks, all in one synchronous turn, so the microtask flush after a reconnect always reconciles the complete battle cache and (d)'s ordering is unreachable today; the contract ships because the pending SDK bump with auto-reconnect is exactly the change that could alter delivery, and (e) is reachable now. **Three review catches worth keeping.** The plan reviewer and the plan red-team independently found that the planned resolved branch dereferenced an undefined row on the COMMON no-battle reconnect, thrown inside the listener and swallowed by its try/catch behind a correct-looking ring — fixed with the null guard, and the suite now carries a global console.error control so a swallowed throw can never hide again. The plan red-team measured that the acceptance gate's exactly-once tooth census stayed green after deleting T3 and T6, the only pre-existing tests that prove the latch RESOLVES under an ignore-signal mutant, so the gate script floors the reseed file at 18 tests. /simplify collapsed the plan's two generation counters into the one boolean — equivalent under every interleaving, no sentinel — and the REARM tooth still kills the reset-inside-the-guard mutant. **A fixture trap for whoever next touches these tests:** the store's latest-battle read returns the HIGHEST id and its upsert never removes rows, so the survivor must be the higher id in every ordering fixture, and a fixture that observes the survivor itself pre-hydration is non-discriminating (silent re-baseline either way). Nine teeth: seven runtime (stale-row-first, ongoing-non-survivor-first, re-arm, no-battle, identity-rotation, orphaned-row, two-pre-hydration-flushes) and two connection.ts source-scan pins (the signal block's presence, contiguity, order, region, writer census and no-early-exit tail; the identity-carrying call site, the two interface members and a shadow-declaration ban). The artifact red-team, writing cheats in a scratch worktree, found three gate-green survivors after every named mutant died — a listener that self-arms on its first pre-hydration sight, a return dead-coding the flush closure, a shadowing identity declaration in the applied callback — and each is now killed by exactly one tooth (measured). **Residuals:** (c) unchanged; (d) and (e) CLOSED; (f) NEW — connection.ts's firing of the hydration callback is proven by text pin only, because the runtime suite mocks the connection and drives the callback itself, so a fire-on-every-flush, a return inserted before the signal, or a shadowing identity declaration is text-caught only; the current-swap race the plan red-team raised was analysed by desync-guard as unreachable (arm and consuming flush share one microtask turn; an earlier attempt makes the arming build stale), and the handler must be total because it runs outside the reconcile try/catch before the flush; (g) NEW — the callback's placement between the ready and reconnect handlers is unenforced and harmless if moved. **Touches-delta:** main.wiring.test.ts (one anchor constant tracks the new handler signature, same assertions), connection.test.ts (append-only), the gate script and ledger in harness memory; docs/knowledge untouched (zero references, verified by grep); CHANGELOG cliff-generated. Boy-scout: one main.ts comment at the menu-leaf identity guard that claimed a reconnect can clear the identity (false; it now reassigns it). ADR next-free = 0234.

**rb-39** (residual R-rb-22-EO-11), ADR-0234 — G5 write attribution walks back from each write verb over its receiver chain and requires the head to be `ctx.db.<accessor>(`; anything else (an aliased handle, a bare local, a UFCS spelling) is Unattributable and fails G5 loud. **This log entry was backfilled by 18r-b, not written by rb-39** — the slice minted and shipped ADR-0234 but added no line to this log, which is why the four entries above it still carry the pre-rb-39 pointer value. The decision text above is recorded from the merged ADR's own header rather than reconstructed from rb-39's slice record, which 18r-b did not measure. ADR next-free = 0235.

**rb-40** (residual R-rb-22-EO-9), ADR-0235. Design B — `privacy::purge_export_bundles` now returns the number of chunks it deleted (`usize`, count bound before the move-loop, tail expression, no `return`; both exact-body pins in privacy_tests.rs and accounts_tests.rs re-frozen with the positive control green); `complete_guest_claim` binds it and emits ONE `guest_claim_export_purge` line through `crate::observability::mr_log` as the TERMINAL statement before `Ok(())` — after `consume_claim_and_disarm` and the AUTH-21 provenance update — because a host log line survives a later panic or `Err` rollback while the deletes do not (both the plan reviewer and the plan red-team independently found the mid-reducer placement and it was moved; a tooth pins the terminal position and bans a `return` token anywhere from the purge binding to the trailing Ok); unconditional (chunks may be 0: the zero-chunk claim is the negative an erasure audit needs, and a conditional is a dead-branch mutant surface); pure private builder `purge_fields(guest, chunks)` (the `heartbeat_fields` precedent, unit-tested by value) — it was planned as `claim_purge_fields` but with that name the call's argument list is 62 columns and rustfmt's default `fn_call_width` (60) lays the call out vertically with a trailing comma that no squashed statement pin can spell, measured on a scratch file, so the builder is the shorter name (args 56, one 91-column line); no bare `log::` anywhere (OBS-2 ratchet untouched, `.log-baseline` byte-identical; the emission goes through the ADR-0180 D6 blessed point); the guest hex is a log FIELD never a label (Loki label set bounded to reducer+evt), the claim-time linkage already persists in `claimed_from` (AUTH-21), PRV1-17/20 applied by analogy (no player-authored field, no name or auth_issuer); rejected: emitting inside privacy.rs (the module's gate-enforced hygiene bans the `log::` token with the reason "the reducer that calls the helper owns any logging", and the owner-generic helper cannot name its cause), an audit table (schema.rs outside touches, ADR-0221 freeze), a countless constant line (observes nothing). Teeth: 8 rb40-prefixed tests (6 RED-arm source scans over the include_str! constants — 5 RED by name on the pre-fix tree, captured at memory/projects/gates/rb-40.red-before.md — plus 2 GREEN-arm behavioural tests that CALL the builder and therefore land with the fix, the rb-22 EO-6 precedent); 793/793 module tests; clippy -D warnings clean. Artifact red-team (write-the-cheat in a scratch worktree): all 33 tester-planned mutants killed, each by a named tooth (nextest runs the filter concurrently, so the first-printed failure is not always the predicted clause — every predicted tooth fires), every candidate cheat (tuple-pattern rebind, block shadow, post-purge guest shadow, named-format-arg override, cfg_attr in place of cfg, operand and closure forms, an in-fn macro_rules indirection, `::log::` prefix spellings) measured closed, and ONE gate-green clippy-clean survivor found: a direct call into the log crate's `__private_api` module (pub mod) — a second unescaped emission channel invisible to every macro-spelling needle; closed in-slice by the tester with a whole-token `log::` ban over both files (mirroring privacy.rs's rb-22 hygiene rule, with a planted-call control), re-measured RED under that clause; the crate-wide G7 ratchet has the same blind spot and is residual R-rb-40-G7PRIVATEAPI. Reviewer (impl): code correct, REQUEST-CHANGES on doc truth only (a misused OBS-1 label in comments, a wrong ADR-0220 decision number, an arithmetic slip, stale test names) — all fixed. Security auditor PASS (the line discloses strictly less, for strictly less time — Loki 30 days — than `claimed_from`, which the cascade never erases; `mr_log_breadcrumb`'s reserved-key debug_assert compiles out of the release wasm); desync-guard PASS (no game rule, no boundary signature, no bindings change). Acceptance ledger: 7/7 met, 0 deferred (E1 + X1..X6; X5 MANUAL = the by-name RED capture). Residuals disclosed (registered via mr-gates): R-rb-40-CASCADE (the cascade and re-export sites discard the count), R-rb-40-ADR0220 (0220 D2 spells the old signature; 0235 Extends not Amends), R-rb-40-ADR0230 (0230's evidence that accounts.rs contains zero mr_log calls is now false; PRV1-17 still holds), R-rb-40-DASH (no panel consumes the event), R-rb-40-G7PRIVATEAPI. Touches-delta beyond the declared set: `docs/knowledge/**` (9 line-stamp pages regenerated), `docs/adr/DIGEST.md` (regen), the two sibling test files; harness memory (plan, ledger, red-before). Boy-scout: one clause in each module header (privacy.rs scan-hygiene paragraph; accounts.rs write-isolation paragraph) and the helper's doc comment. `lib.rs` is in touches but needed no edit. ADR next-free = 0236.

**18r-b** (`M-postgate-eighteenth-review-residuals`), **no new ADR — none was reserved and none is warranted: doc/comment-only, zero schema, zero reducer, zero client behaviour.** A citation-truth micro-sweep closing four stale pointers, on rb-36's doctrine that the bare line number is itself the defect. (1) ADR-0231's Consequences bullet cited a `main.ts` line that had drifted onto an unrelated `onSessionExpired` callback; it now names the `onClaimResult` callback's AUTH-51 / D15 claim-rejected branch as the anchor and carries the number only as an explicitly dated hint. (2) `sim-harness/src/bin/mr_load_driver.rs` placed `on_disconnect` at a `server-module/src/lib.rs` line RANGE that m22-s3b had since turned into `resolve_all_live_interactions`; it now names the `client_disconnected` reducer with no line number at all. That replacement is exactly line-count-neutral, because ADR-0232 cites this comment block BY RANGE as its live-verified evidence and sits outside this slice's `touches:` — growing the block would have created, in a file this slice may not edit, the very defect class it exists to close. On the reducer-security-auditor's finding the sentence also now records that the reducer force-resolves live trades/PvP/battles BEFORE the row deletes, closing a "a stray HTTP call is harmless" misread. (3) rb-39 minted ADR-0234 but wrote no entry in this log; it is backfilled above, inserted CHRONOLOGICALLY rather than appended, so rb-40's entry stays terminal and a reader taking the last entry's pointer still gets the current number. (4) `AGENTS.md` said the workflows pin the spacetime version in three places; it is four, and the wording now names the oracle so the count is re-derivable. **A planned fifth edit was CUT:** a clause generalising that the per-slice next-free notes are point-in-time and never back-edited — a red-team MEASURED that false (two entries were rewritten by later, unrelated slices), so the explanation was localised into the backfilled entry instead of shipped as a rule over a class. The acceptance gate `memory/projects/18r-b.gates.mjs` is harness-side because ADR-0224 bars new `evals/` scanners: five arms deriving every expected value from the live tree, RED before the edits and green after, with six mutants each RED on their own named check — including a content swap that preserves the section's line count (invisible to the range check alone) and a terminal append of the backfilled entry. Known limit, carried as a residual: the stale-citation class itself stays ungated in `just ci`. ADR next-free = 0236.

**rb-46** (residual R-m22-s5-X12), ADR-0236. M22 §4.7 named PvE battle start and shop buy/sell as deletion-gate targets ("shop buy/sell is DECIDED IN" because `player_wallet` and `inventory` are ERASE-policy tables), but `battle.rs` and `economy.rs` were outside m22-s5's `touches:`. The existing caller-only wrapper `guards::require_not_deleting` (unchanged, byte-pinned by m22-s5 tests, delegating transitively to `accounts::should_reject_for_deletion`) is now called as the single statement `crate::guards::require_not_deleting(ctx, "<own name>")?;` in four reducers: `battle::start_battle` (after pure party-size caps/ADR-0048 provenance/dedup checks, immediately before the first DB read `is_in_ongoing_battle`), dev-reducers-only `battle::start_wild_battle` (after the joined check; ledger gate X6), `economy::buy` and `economy::sell` (immediately after `require_owner`, before qty checks). The crate-wide caller set grows from four (trading.rs, pvp.rs ×2, ranking.rs from m22-s3b) to eight; the m22-s5 "exactly three" census is correct as a scope-local claim. Enforcement: ADR-0227 D6's premise "reducer bodies have no runtime harness" is stale since rb-41's native_host_tests. Three BEHAVIORAL tests run the shipped reducers under the native host through a five-state progression (no account row / Active / PendingDeletion / PendingDeletion+terminal marker / row removed) with the exact verdict pinned in each state; four per-site SOURCE PINS cover qualified path, `?;`, brace depth 0, statement boundary, body-wide `#[`/`cfg!(` ban, bare-name count 1 (kills a sibling `require_not_deleting_for(ctx, opponent_identity)` — invisible because the native host's sender is the all-zero identity == WILD_IDENTITY), every early exit is a rejection (count of `return` == count of `return Err(e);` — kills depth-0 `if me != WILD_IDENTITY { return ungated_twin(..) }` or file-scope `#[cfg(debug_assertions)]` const steering returns), anchor uniqueness. One CENSUS test in guards_tests.rs: exact gated set per file, whole-file bare-name count two, per-site log tag inside its own reducer's declaration region (kills buy/sell tag swap), bypass bans (is_pending_deletion / should_reject_for_deletion / ctx.db.account( / cfg_attr( / renamed reducer attr). Gates: 8 rb46_ tests RED at HEAD for predicted reasons, GREEN after; `just ci-fast monster-realm-module` green (808/808 tests), `just eval` green (99 evals incl. knowledge-bundle-conformance + adr-digest), 22-row mutant register executed one mutant at a time by `memory/projects/gates/rb-46.mutants.py` — 22/22 caught, control 21/21, tree restored byte-for-byte — with the per-row record in `memory/projects/gates/rb-46.red-before.md`; the full `just ci` run and the acceptance ledger (E1 seeded + X1-X6) are recorded in `memory/projects/gates/rb-46.gates.md`. Docs: ADR-0236 (new; Extends ADR-0227), ADR-0227 amended with dated discharge of "still-ungated §4.7 targets" bullet, ARCHITECTURE.md m22-s5 paragraph boyscout-corrected ("now gates exactly the three"), docs/knowledge regenerated, DIGEST regenerated. Residuals registered: R-rb-46-GRASSPATH (scheduler grass path movement_tick → begin_encounter still opens wild battle for mid-grace walker; keyed on server-derived player_identity, out of touches), R-rb-46-ERASEWRITERS (§4.7's trigger also selects heal_party, advance_dialogue quest grants, taming grant_item path — ungated, out of touches), R-rb-46-TRADINGCFG (m22-s5 pins on trading.rs lack `#[cfg`/statement-boundary clause; pvp.rs covered by ranking-security's body-wide `#[cfg` ban), R-rb-46-MACRORET and R-rb-46-LIBRSMOD (evasion classes: macro_rules! conditional return; lib.rs cfg(target_arch)-selected `mod guards` twin). Known limits: `Fixture::table` keys rows by Identity bytes so u32/u64-keyed content cannot be seeded (RED is "gated caller admitted past caller-standing checks into content lookup", not "wallet debited"); every write syscall aborts the process (gate-after-write owned by source pins); fixed all-zero sender makes sender-keyed bypass direction owned by pins, reject-everyone direction is e2e/playtest-visible outage. rustfmt kept all four call sites inline. Touches-delta beyond declared: docs/knowledge/** (generated), docs/adr/DIGEST.md (generated), ARCHITECTURE.md. guards.rs declared but unedited. ADR next-free = 0237.

**rb-47** (residual R-m22-s5-X13), ADR-0237. ADR-0227's confederate role-swap residual: D requests deletion, confederate C proposes a trade TO D afterwards (the caller-only gate passes for C by design, ADR-0227 D4), D calls the ungated `respond_trade(accepted = true)`, C confirms — a new commitment mid-grace. `respond_trade` now carries ONE depth-0 statement after `authorize_respond` and after the decline block, before the status update: `crate::guards::require_commitment_predates_deletion(ctx, "respond_trade", offer.created_at_ms)?;` — a stamp-conditioned gate (refuse iff the caller is deletion-gated and the offer was created at or after their own request; terminal marker refuses at every stamp; missing stamp refuses), never a blanket one, so predating offers stay completable (PRV1-10) and declines stay open. Seam: pure `accounts::opened_commitment_is_refused(&Account, opened_at_ms)` delegating the mid-grace arm to `should_reject_for_deletion`, ctx-bound `accounts::refuses_commitment_opened_at(ctx, identity, opened_at_ms)` (a second identity-taking primitive whose ONLY consumer is the caller-only guards wrapper — contained by both `guards_tests.rs` bypass arrays, 5→7 each, a guards.rs single-consumer clause and a crate-wide containment test derived from `lib.rs`'s module declarations), and the fused wrapper reusing `deletion_gate` + `REJECT_DELETION_GATED`. Enforcement: ten `rb47_` tests in `trading_tests.rs` — the pure truth table (negative offsets, non-zero request stamp, varied fixtures), the ctx predicate and the wrapper executed under the native host (five states × three offsets, stranger row, stranger-identity calls), the SHIPPED reducer executed over a u64-keyed `trade_offer` via the new additive `Fixture::table_keyed` (refused direction only — the write wall), whole-body EQUALITY pins on all three seams plus `deletion_gate` (the artifact red-team measured containment/`starts_with` pins CI-green against clock-keyed, `cfg!(test)`-keyed, issuer-keyed and trailing-combinator mutants), declaration + `#[cfg`/`#![cfg` whole-file counts in accounts.rs/guards.rs/trading.rs, the call-site pin with the whole statement INCLUDING the argument, prefix equality above the gate, and the strings-intact tag, propose-time stamp provenance (`created_at_ms: now_ms(ctx),` once), a closed reducer roster for trading.rs, and the R-rb-46-TRADINGCFG hardening of the propose_trade pin. 46-row mutant register (36 planned + 10 red-team survivors) executed one at a time on a source copy by `memory/projects/gates/rb-47.mutants.py`, record in `memory/projects/gates/rb-47.red-before.md`. Residuals: R-rb-47-PREDATING (offer proposed before the request — admitted by the immutable EARS, TTL-bounded), R-rb-47-CANCELLAUNDER (cancel-accept-re-request — costs a fresh grace period), R-rb-47-ROSTER-PVP (the closed roster covers trading.rs only). Touches-delta: accounts.rs, guards.rs (production; the fix's disclosed hidden dependency — no sibling in flight), guards_tests.rs (arrays + prose, zero assertion changes), native_host_tests.rs (additive key-generic fixture), docs/knowledge/** + DIGEST regenerated. Docs: ADR-0237 (new; Extends ADR-0227), ADR-0227 amended, this paragraph and the m22-s5 paragraph + guards.rs row above. ADR next-free = 0238.

**17r-e** (comment-truth micro-sweep), **no new ADR — none is warranted by ADR-0224: comment/detail-string only, zero behaviour change, zero schema, zero reducer, zero client code logic.** Two files changed, both comment/description only; `just ci` green (2224 Rust tests, 2943 client tests, 99 evals, clippy `-D warnings` + biome + fmt clean). (1) **`client/src/ui/overlayA11y.ts:51-54`** — the module header's cross-slice contract **(a) / A12** claimed that `battleView`, `boxView`, `raisingView` and `evolutionView` "share ONE `#app`-mounted root", prescribing that "S4 must CLOSE-BEFORE-OPEN" because "opening the next id first stacks two capture traps on the same node". **Factually false.** Each view creates its OWN root via `document.createElement` and appends it into the shared mount (`battleView.ts:81/171`, `boxView.ts:60/120`, `raisingView.ts:78/124`, `evolutionView.ts:68/97`), `OPEN_OVERLAYS` is a `Map<OverlayId, OpenRecord>` with four distinct OverlayId keys (`'battleView'`, `'boxView'`, `'raisingView'`, `'evolutionView'`), yielding four distinct records that never collide, and `installTrap` attaches its capture listener to the passed `root` parameter (`focusTrap.ts:150`), not the mount, so two traps cannot stack on the same node. The retracted claim is withdrawn in place and the true shape stated with a pin to the distinguishing marker `S4-CROSS-VIEW-DISTINCT-ROOTS` (`boxView.test.ts:287`). The edit is **line-count neutral (4 lines in / 4 lines out, confined to lines 51-54)** because `overlayA11y.ts` is cited by line number in 102 locations across 18 files outside this slice — a header insert would silently drift every one of them, and nothing in `just ci` measures that, so gate E3 pins line-count neutrality instead. (2) **`evals/playtest-report.eval.mjs` — five sites** dropped the stale `EXPECTED RED until the specialist's identity-contract tightening lands` qualifier from the file header (`:47-48`), the T3p and T3q block comments (`:1358`, `:1392`), and the two failure-detail strings (`:1384`, `:1417`). The tightening landed: `scripts/playtest-report.mjs:coerceRow` (:132-154) now requires `identity` to be a one-element array holding a non-empty string; the `typeof rawIdentity === 'string'` accept branch no longer exists — the `if` condition at `:133-138` rejects any non-array, any wrong length, any non-string element, or any empty string, all with fail-loud throws. The Section-3 summary echo at `:1859` was deliberately left byte-unchanged (already accurate, no " EXPECTED RED" text there). **Four registered residuals, disclosed not fixed, all outside this slice's `touches:`** — (a) **R-17r-e-B1** — the two `.ron` comment falsehoods (`game-core/content/species/070-wave3.ron:17` "Electric resists nothing but its own mirror" — `game-core/content/type_chart.ron:9` Electric→Electric 0.5× **and** `:25` Water→Electric 0.5×, so Electric resists both; `game-core/content/species/071-wave3-derived.ron:36` "Tempestrix owns the Regeneration pivot" — three species carry `ability: Some(3)`: `000-core.ron:24`, `020-playtest-wave1.ron:64`, and `051-wave2-derived.ron:39`). Blocked because `evals/content-version.eval.mjs:31-40` raw-byte-hashes `game-core/content/**` against `evals/baselines/content-hash.json` (v21) + `server-module/src/lib.rs:79` `CONTENT_VERSION = 21`, both outside this slice's `touches:`. (b) **R-17r-e-E3** — `client/src/ui/focusTrap.ts:58-62` independently repeats the retracted claim as a **live prescription** ("S4 must close-before-open"). An implementer reading `focusTrap.ts` instead of `overlayA11y.ts` would act on false instruction. Outside `touches:`. Highest-value remaining carrier. (c) **R-17r-e-VIEWHDR** — `battleView.ts:29-30`, `boxView.ts:29-30`, `raisingView.ts:30-31`, `evolutionView.ts:40-41`, and `ARCHITECTURE.md:1908` (the `**m23-s1**` block) each describe contract (a) in the PRESENT tense ("contract (a) *says* the four views share ONE root"). After this retraction, that is no longer what (a) says, so all five are now stale on tense (not on substance — all already call the claim a misstatement). (d) **R-17r-e-UPDATEFLAG** — `evals/content-version.eval.mjs:109` advertises a `--update` flag the file does not implement. **Why no ADR:** this slice adds no dependency, no production pattern, no shipped decision — ADR-0224's ratchet forbids manufacturing an ADR for a comment sweep. The retracted claim was S4's own discovery and correction (pinned in `boxView.test.ts`), not a pre-existing documented decision. ADR next-free = 0239.

**rb-54** (residual R-m23-s8-postmerge), ADR-0239. `game_core::validate_a11y_tokens` derives its required a11y-token-key set from two HAND-MAINTAINED rosters (`STATUS_KIND_ALL: [StatusKind; 5]`, `Affinity::ALL: [Affinity; 8]`) mapped through exhaustive key functions, so a new enum variant is a compile error at the key function but **not** at the roster — the key never enters `required`, `A11Y_TOKENS` is never asked for a row, and `validate_content` returns `Ok(())` at content-sync time. Only a game-core `#[cfg(test)]` test caught it. `server-module/src/content.rs` now carries a production sibling validator: `EnumRosterTypespace` (a never-interning `TypespaceBuilder`), `reflected_variant_names<T: SpacetimeType>` reading the DERIVE-GENERATED variant list, `check_roster_is_total<T>(enum_name, roster)` which reflects from `T` internally (no `reflected` parameter, so the roster's element type binds the oracle) and asserts `roster.len() == reflected.len()` plus pairwise distinctness, and `validate_enum_rosters()` — called as the FIRST statement of `sync_content_inner`, **above** the `content_version` early return, because the predicate is data-independent and below the gate it would be skipped on every already-current database. Zero new dependencies: `server-module` already enables game-core's `spacetimedb` feature, so the derive metadata is reachable from production code (`ron`/`serde` are not server-module deps and `Cargo.toml` was outside `touches:`). **Measured end to end:** with `StatusKind::Frostbite` genuinely added and the roster left at five, adding the variant forces FOUR `E0004` arms (not the "one" the upstream note at `game-core/src/content.rs:7490` claims — that note is stale); the workspace then compiles, `content_parses_and_validates` (i.e. `validate_content`) still PASSES, and the new gate fails with a message naming the missing variant. Teeth: 14 `rb54_*` tests; 22-row mutant register, 22/22 caught, tree sha-verified restored after every row. The plan-phase red-team measured 8 CI-clean bypasses of the original design (the `_with` injectable seam was deleted in response, binding the oracle to `T` instead); the ARTIFACT red-team then measured 3 more that all 10 original teeth survived *with a real unrostered variant present* — a label-keyed oracle, a `TypeId`-keyed redirect inside the unpinned `TypespaceBuilder::add`, and a hardcoded per-enum count — because only the trivial function was body-pinned. Four closure teeth (J/K/L/M) pin `add`'s body to `make_ty(self)`, ban a concrete `game_core::` type inside the oracle's body, pin the length comparison to `reflected.len()`, and behaviourally reflect `Affinity` under the label `"StatusKind"` so a label-keyed oracle diverges. **Honest bound, recorded in ADR-0239:** the predicate is data-independent and the host suite evaluates the same function, so any CI-green artifact necessarily has `validate_enum_rosters() == Ok(())` in the shipped wasm — the reducer wiring cannot fire in a CI-green build. A new variant also already reddened two pre-existing detectors (`m23s8_totality_*` and `evals/spacetime-type-snapshot.eval.mjs`); the value added is a third detector living in the module itself plus a sync-time failure with an actionable message. Deferred: `R-rb-54-STATUSEFFECT-PAIR` — the `StatusKind`/`StatusEffect` set-equality invariant is a different kernel (two reflected name lists, no roster walk) whose `PartialEq` roster contract does not hold for a payload-carrying enum. Touches-delta: `docs/adr/0239-*.md` + `DIGEST.md` (regen), this paragraph. ADR next-free = 0240.

**rb-55** (residual R-m23-s8-TSDUP), ADR-0240. The five status a11y badge tokens were hand-written twice — `A11Y_TOKENS` in `game-core/src/content.rs:1692` (the A11Y-29 SSOT) and a `switch` in `client/src/ui/battleModel.ts` — with nothing correlating them; ADR-0233 recorded the gap and left it to a doc comment. **Measured on master: four of the five could be renamed on the TS side with the whole client suite green** (`PSN` alone was incidentally pinned by a regression assertion at `battleModel.test.ts:2201`, so the honest count is four, not five). One `describe`/one `it` now reads the `A11Y_TOKENS` const body out of `content.rs` and asserts that `statusBadge()` RETURNS, for every generated `StatusEffect` variant, that variant's shipped token — and re-asserts the same through `buildBattleViewModel`, so the value a card actually carries is bound too. **The oracle is deliberately the return value, never a constant the file merely contains:** three mutants (an early-return interceptor, an unwired parallel table, `.toLowerCase()`) survive a contains-the-string oracle with the suite green. No token literal appears in the new block — the point is to stop making hand-copies, not to add a third. **Production logic is UNCHANGED in both languages; the Rust and TS edits are comments only.** A planned `switch` -> exported-data-table refactor was CUT by `/simplify`: once the oracle is the function, the table bought only a dead `case` arm for a variant the server cannot send, while introducing a prototype-chain hazard (`statusBadge('toString')` returning a function that `battleView.ts:290` would render) that then needed its own tooth. Enforcement: a 16-row mutant register (12 CAUGHT / 3 CONTROLS GREEN / 1 PROBE red / 0 survived, tree sha-restored after every round), each desync mutant recorded with its PRE-SLICE verdict so the red half of red-green is evidenced rather than asserted. The **artifact** red-team then measured four bypasses that kept the entire gate set green — a raw-string decoy reproducing the anchor line with its own `];`, a `#[cfg(any())]`-disabled row beside a `concat!`-spelled real row, a block-comment carrier, and a `.replace()` at the `monsterCard` hop — all four now RED via anchor-uniqueness, row-totality, attribute/macro and block-comment bans, and the view-model assertion. A ninth-affinity control confirms a pure-Rust content decision does NOT false-RED the client suite. **The `content.rs:1621-1625` and `:7513-7515` comment rewrites are exactly line-count-neutral**, because ADR-0239 and `server-module/src/content_tests.rs` cite this file by line and both sit outside this slice's `touches:` — a first attempt grew `:7513` by two lines and silently broke four ADR-0239 citations, caught by the reviewer. **Honest bound: this closes the CORRELATION half of the residual, not the DEDUPLICATION half** — the client `switch` is still hand-maintained, and deriving it needs an a11y-token table, a bindings regen and an addition to the exact-set-pinned client subscription set, all outside `touches:`; DEFERred to backlog in the ledger. Known gaps, registered not papered over: the final DOM hop (`battleView.ts:290`) is unguarded, and the gate itself is a one-hunk deletion with no CI signal — the `include_str!` existence pin that would close it was rejected as the meta-check ADR-0224:133-141 retires. ADR-0233:179-182 keeps a now-stale sentence, unowned: editing it exceeds the one-new-ADR grant. Touches-delta: `docs/adr/0240-*.md` (new) + `DIGEST.md` (regen), this paragraph. ADR next-free = 0241.

**rb-58** (residual R-m23-s8-postmerge-fallback == R-m23-s8-FALLBACK-COLLIDE), ADR-0242. `unknownStatusToken` (`client/src/ui/battleModel.ts`) derived the unknown-status badge from only the first two code points of the tag, so `Confusion` and `Corrosion` both rendered `?CO`; it now folds a polynomial rolling hash (multiplier 31, modulus 1296 inside the loop) over EVERY code point into two base-36 digits, keeping the `?` prefix and the 3-character budget. **Honest bound: 1296 tokens is the pigeonhole MAXIMUM for that budget, so collisions REMAIN POSSIBLE (~0.077% for any given pair, even odds at roughly 43 concurrent unknown tags) — what changed is that they stopped being SYSTEMATIC on a shared prefix, and no residual is closable without reopening the 3-character layout budget; ADR-0233:195-198's now-false paragraph is DEFERred to backlog as gate X4 rather than amended, since a second ADR is outside this slice's grant.** Enforcement is five vitest teeth (`rb58 T1..T5` in `battleModel.test.ts`), no new eval per ADR-0224, and the 27-row register at `memory/projects/gates/rb-58.mutant-register.md` measured 25 CAUGHT / 0 SURVIVED / 2 controls held, run in full three times — 12 rows die to exactly one tooth, and 11 rows were CI-clean bypasses that were MEASURED: nine by an adversarial pass over the tests before any implementation existed (two of them, M13/M20, leaving the function textually spotless and confirmed by a real `vite build` to tree-shake the ORIGINAL colliding transform back into the production bundle), and two more by the verifier against the finished artifact after six lenses had passed it — a wrong-side zero-pad, and a `statusBadge` call-site truncation at any cut point past the acceptance corpus's own 10-code-point maximum, which is why `rb58 T4`'s ambient-read ban is file-wide and `battleModel.ts`'s purity is now a standing enforced property. rb-56 and rb-57 added no entry to this log, so the gap between rb-55 and this paragraph is intentional rather than a missing record (rb-57 shipped ADR-0241; rb-56 minted no ADR). ADR next-free = 0243.

**rb-60** (residual R-rb36-ADR0206CITE), no new ADR — prose-only, zero behaviour change. rb-36 retargeted seven drifted `main.ts` citations onto stable landmarks but could not touch this one: `docs/adr/**` is admitted to a slice only for its own reserved ADR number, and rb-36 had none. ADR-0206's "Accepted residual — `visibleIds(probes)[0]`" paragraph cited `client/src/main.ts:1574` (plus a bare `:1565`) for two claims — dialogueView is rendered unconditionally on every store batch, and menuView is the only force-hide. Both numbers had drifted off the code they name (`:1574` is now an unrelated Escape/`tradeProposeView` branch); the claims themselves were re-measured and are still TRUE, so this is a citation repair and not a retraction. Retargeted per rb-36's citation doctrine — the named landmark is authoritative, the number a dated hint — onto main.ts's M12d `store.onBatchApplied` dialogue listener (`:1837-1887` today, with `:1851` today the render and `:1842` today the force-hide). **Boy Scout, same paragraph, same defect class:** the sibling `client/src/ui/overlayRegistry.ts:372-374` citation landed inside `hideAllExceptPlan`'s JSDoc; retargeted onto `visibleIds` at `:400-402` today. Paragraph re-wrapped to ~100 cols; a word-level diff confirms nothing but the citations changed. Per ADR-0224 a prose-only ADR citation fix takes no new eval and no new shipped test, so acceptance is a harness-side ledger oracle (`memory/projects/gates/rb-60.oracle.cjs`) that INVERTS the usual direction: it parses the line numbers out of the ADR prose and uses the cited sources as the oracle, so the gate holds no drifting copy of the fact and cannot red on an unrelated `main.ts` edit. Measured RED pre-fix, GREEN post-fix; 40-row mutant register 37 caught / 3 controls green / 0 survived, after FIVE measured survivors were found and closed. Two were scope errors — a paragraph-wide `today` check survived de-hedging one citation, then a fixed-width 60-char window survived by reaching into the NEXT citation's hedge (the window now ends at the next code span): a presence test over a shared region proves presence *somewhere*, not *at the site*. Three more were found by the ARTIFACT red-team AFTER the plan red-team, the reviewer and a 20-row register had all passed the gate — the cited RANGE was only checked for CONTAINMENT, so `hi` could be widened past the listener's real close into the next declaration, `lo` dragged back onto a different single-line `store.onBatchApplied(...)` statement, and the boyscout range widened past `visibleIds`'s own `}`. Closed by requiring the cited range to be the landmark's EXACT extent (module-scope opener ending in `{`, first indent-0 terminator as the close, none swallowed in between; brace COUNTING deliberately avoided — strings, template literals, regex literals and comments all desync a naive counter). A `/simplify` lens then mapped every check to its justifying rows and found the `visibleIds`-contains check dead (`exactExtent` already asserted the same pattern on the same line) plus a latent honesty gap — the PASS summary printed the LITERALS `stale-1574=absent hedge=4/4` instead of values derived from the checks that ran, so gutting either check would have left the summary still claiming it held; both fixed, and the four guards no row can exercise are now declared in the register rather than quietly kept. **Declared, not fixed:** a 4-of-4 spot check of ADR-0206's other ~35 `main.ts:NNN` citations found all four drifted by ~250-280 lines (ADR lines 17, 19, 48, 93) — treat that whole citation set as suspect; a mass recitation is its own slice, far past a boyscout cap. Also noted: rb-59 (#444) merged without an ARCHITECTURE.md entry. ADR next-free = 0243.

**rb-64** (residual R-rb-39-PRIVACY-LOCAL-PORT), no new ADR — no number reserved; the design decision lives in ADR-0234, which this slice executes for a second file. `privacy_tests.rs`'s local `rb22p_write_targets` is now a verbatim-equivalent local copy of ADR-0234's `write_target_accessors` + `rooted_chain_accessor` (privacy_tests.rs:406 and :451), returning `Vec<Result<String, WriteAttrFault>>` with three typed fault variants (privacy_tests.rs:348); the three `RB22P_*` poison consts (master `:337-339`) are deleted. Caller `rb22p_writes_only_export_bundle` binds `[W/non-vacuity]` to `targets.iter().any(|t| t.is_ok())`, `[W/attribution]` to `for t in &targets` (panics on any Err), `[W/target]`/`[W/arithmetic]` to `targets.iter().flatten()`, and `[W/total]` to `targets.len()` (measured: flatten spelling collapses tooth from 18/18 caught to 2/18; privacy.rs census remains 5 writes, 3 export_bundle + 2 reaper_schedule, zero faults). **RED-before (harness `memory/projects/gates/rb-64.red-before.md`, measured on the pre-port helper before any edit):** F4 the residual's exact case `if ctx.db.export_bundle()...is_some(){db.account().identity().delete(v)}` returned `["export_bundle"]` (misattributed to owned table inside allowed set); F5/F6 silent drops (no try_insert, no UFCS in prior verb list). **Measured red-team findings:** plan-phase 10 of 14 cheats passed original four tests; seven fixtures P1-P7 kill all; artifact-phase census-shape cheats (drop-a-refusal-once-non-empty, cap-at-5, return-at-first-refusal; register M16-M18) closed by T8 rows. Two verb spellings CI-clean after port: fn-item trait-verb binding (`let del = <_ as Table>::delete;`) and in-file declarative macro with verb metavariable (`macro_rules! w { ($r:expr,$m:ident,$a:expr) => { $r.$m($a) } }`) produce no census entry in either walk and both shipped 858/858 Rust tests + 99/99 evals (artifact red-team, `rb-64.lens-measurements.md` B); closed for privacy.rs by total bans in T9. Reducer-security-auditor re-measured rb-39's probe against the shipped gate (probes A/C/D, verbatim in `rb-64.lens-measurements.md`): `[W/attribution]` catches UnrootedChain; with the alias removed (a column handle bound one statement earlier, `rb22p_no_db_or_ctx_alias` green) the walk is the sole tooth that catches it, where the pre-slice gate credited the delete to export_bundle and failed only on rb-48's exact-count arithmetic. **Hidden dependencies declared (NOT edited; residuals to register after lenses):** ADR-0234:38-43, :51-56, :70-72 (the last already false on master: privacy.rs has held writes outside the pinned purge helper since m22-s4/rb-48), :152-155, and its in-file-macro honest-limit bullet (now false by measurement); ADR-0226:171 ("scans only DOTTED verbs"); accounts_tests.rs:13712-13714 (present tense about the `;` rule) and :14478 (cites `rb22p_scan_hygiene` at privacy_tests.rs:711, drifted); docs/plans/m22-s4-plan.md:260; split-binding control (`[W/split-binding]` in evals/guest-claim-integrity.eval.mjs) scans accounts.rs only so per-table handle to privacy.rs sibling is ungated (measured by the reducer-security-auditor, probe E in harness `memory/projects/gates/rb-64.lens-measurements.md`: a foreign `account` delete reached through a sibling-module fn passed the whole Rust suite, `858 tests run: 858 passed`); paren-less verb + in-file macro blind spot in shared helper (same shape in accounts_tests.rs; accounts.rs has no close, privacy.rs now does via total bans). Boy-scout: m22-s4 banner's "everything above is unchanged" was already false on master, now accurate; false "body is pinned" claims deleted. Teeth: nine `rb64p_` tests (T1-T9) over fixture rows; X1-X5 ledger gates; bite-proof register (harness `memory/projects/gates/rb-64.mutant-register.md`): 18/18 mutants killed, each by the predicted test — 13 walk mutants including the restored pre-port body, 2 privacy.rs probes for the paren-less and macro spellings, 3 census-shape cheats. ADR next-free = 0243.

**rb-62** (residual R-rb36-WIRINGCITE), **no new ADR — none was reserved for this slice, and none is warranted: no dependency and no production pattern is added** (the rb-15/rb-17/rb-18/rb-36/rb-37/rb-60 precedent for a prose-only residual). **Comment/prose only.** Two files: `client/src/ui/overlayA11yWiring.test.ts` (the declared `touches:` set) and `ARCHITECTURE.md`. Net -4 lines in the spec file, 0 assertions touched, 123/123 tests in that file before and after, full `just ci` green (nextest 2247/2247 passed 0 skipped, vitest 3142 passed across 107 files). **The residual's premise was HALF ALREADY CLOSED before this slice started.** rb-37 (PR#415, 2026-09-02) boy-scouted this same comment and its own `ARCHITECTURE.md` entry (line ~2202) already claims to close R-rb36-WIRINGCITE — it did fix the false "flagged, not touched" meta-citation, but kept a dated `:1627-1641` line hint, and rb-52 then inserted ~200 lines into `main.ts` and drifted it. The mr-gates residual row stayed open because it was derived from rb-36's disclosure, which predates rb-37. So rb-62 closes the ledger row and fixes the second-order number drift that rb-37's own hint reopened. **The fix: retargeted onto main.ts's M12d `store.onBatchApplied` dialogue listener — `:1837-1887` today** (banner at `:1835`, opener `:1837`, first indent-0 close `:1887`), with the `buildDialogueViewModel(...)` call at `:1850` today cited as the line the fresh-view-model-per-batch fact rests on, and `dialogueView?.render(dialogueVm)` at `:1851` today. Same citation doctrine as rb-36/rb-60: the named landmark is authoritative, the number a dated hint. The fact was re-measured and is still TRUE, so this is a citation repair, not a retraction. **THE LOAD-BEARING DECISION: the trailing meta-citation parenthetical was DELETED, not rewritten.** The /simplify lens made the argument and it was accepted: this residual exists BECAUSE a meta-claim about two other files went stale, so authoring a corrected one re-arms the identical trap — a second time, since rb-37 had already rewritten it once and it drifted again. A code comment in a spec file is the wrong home for slice archaeology; the SSOT for it is this log (rb-18 at :2198, rb-36 at :2200, rb-37 at :2202, rb-60 at :2226). This entry is what preserves the chain, which is why it exists. **Boy Scout, same defect class, one line: ARCHITECTURE.md:1997** narrated the same fact with the same drifted `:1627-1641` hint; retargeted onto the same landmark. **ACCEPTANCE, per ADR-0224** (no new eval, no new shipped test for a prose fix): a harness-side ledger oracle, `memory/projects/gates/rb-62.oracle.cjs`, in the INVERTED direction — it holds no copy of the fact, parses the cited numbers out of the prose and uses `main.ts` as the oracle, so it cannot drift and reds only when an edit moves the landmark out from under the citation. Ledger 2/2 met, 0 deferred. Measured RED pre-fix (naming the real defect), GREEN post-fix; 23-row mutant register 23/23 caught, 4/4 controls green (including a comment re-wrap control, since the oracle normalises the `//` block before matching). **The oracle was hardened after measured bypasses,** and that is worth recording because each is a reusable lesson: (1) a block-scoped stale-citation check was bypassed by a single blank line — breaking the contiguous `//` run parks a bare citation outside the checked region, so the check is now whole-file; (2) blacklisting the string `main.ts:1574` was bypassed by rewording it as `main.ts's line 1574`, so the ban is now on the drifted DIGITS, which closes every spelling at once; (3) matching `ARCHITECTURE.md`'s narration non-globally let a second, differently-numbered narration survive further down the append-only log, so it is now count-and-agreement checked; (4) the first draft's PASS summary printed the literals `hedge=2/2` and `meta=2/2` — the exact regression rb-60's /simplify lens had already caught once — and both were measured green against a gutted check, so every printed value is now derived. **DECLARED, NOT FIXED** — registered as residual R-rb-62-R-rb62-DIALOGUEVIEWCITE (target: backlog), not left in prose: the same drifted `:1627-1641` hint is still live at `client/src/ui/dialogueView.ts:17` and `client/src/ui/dialogueView.test.ts:243`, `:290` and `:374` — four sites, grep-confirmed as exactly and only those. Both files are outside rb-62's declared `touches:` set, so they were flagged rather than edited. The `rb-62.oracle.cjs` shape generalises to them by changing its `SPEC` constant. ADR next-free = 0243.
**rb-61** (residual R-rb36-FOCUSCITE), **no new ADR — none was reserved for this slice, and none is warranted: no dependency and no production pattern is added** (the rb-15/rb-17/rb-18/rb-36/rb-37/rb-60/rb-62 precedent for a prose-only residual). **Comment/prose only — zero behaviour change, zero production logic touched.** One declared file, `client/src/main.a11yFocus.test.ts`, +5/-3 (net +2 lines), 0 assertions touched. This closes the LAST of rb-36's three same-class residuals (R-rb36-WIRINGCITE -> rb-62 #447, R-rb36-ADR0206CITE -> rb-60 #445). **The defect was TWO defects, not the one rb-36's disclosure named.** `main.a11yFocus.test.ts:782`'s `S5T-BODY-BLUR` rationale cited `main.ts:1574` for the trigger of the dead-hotkeys-after-a-dialogue bug; that line is today an unrelated `tradeProposeView` Escape branch (pt-c2 / ADR-0134 D7). **AND the call it quoted, `dialogueView?.render(null)`, exists NOWHERE in the repo** — a fabricated call spelling, which no amount of line-number repair would have caught. Retargeted onto main.ts's M12d `store.onBatchApplied` dialogue listener (`:1837-1887` today) with the real call `dialogueView?.render(dialogueVm)` (`:1851` today), whose `dialogueVm` is null once the conversation row is gone, so `DialogueView.render`'s `!vm` branch display:nones the overlay holding the focused choice `<button>`. Same doctrine as rb-36/rb-60/rb-62: named landmark authoritative, number a dated hint. `store.onBatchApplied(` occurs **15 times** in `main.ts`, so the listener name alone is not a landmark; the unique disambiguator is its catch tag `[M12d] dialogue batch listener error`. **Only ONE line number survives in the shipped prose**, down from two in the draft — a `/simplify` finding, adopted verbatim: a second number is a second copy of the same drifting fact. **ACCEPTANCE per ADR-0224** (no new eval, no new test for a prose fix): a harness-side ledger oracle, `memory/projects/gates/rb-61.oracle.cjs`, INVERTED — it parses the citation and the quoted call OUT of the prose and verifies them AGAINST `main.ts`/`ui/dialogueView.ts`, so it holds no number of its own to drift. Measured RED pre-fix, GREEN post-fix; 10/10 mutants caught with 2 controls green. Ledger 2/2 met, 0 deferred; `just ci` CI-EXIT=0. **THE HONEST BOUND, stated because a PASS will be read as more than it is: the oracle gates CITATIONS, not CLAIMS.** Three red-teamed mutants print a byte-identical PASS while narrating a false story around correct citations (an inline self-contradiction, a fabricated cause, an unrelated causal disclaimer). This is structural — hardcoding the one correct narrative would be a second copy of the drifting fact, and promoting the comment into an `expect(` is exactly what ADR-0224 forbids here. **Every review lens changed the artifact, and each lesson is reusable.** PLAN phase: red-team measured that the planned span-walk direction collected ZERO lines, so the gate could never have passed even on a correct fix — the same class as rb-60's "gate can never go green" trap; and a backwards-mechanism mutant passed a presence-only check, closed by asserting the ORDER of the `!vm` and `display:none` clauses. ARTIFACT phase: red-team measured three more — a stale citation evades a census that requires the `client/src/` prefix; the quoted call can be pulled OUT of the paragraph and planted elsewhere in the file (closed by requiring the SPAN itself to carry its evidence, not merely the file); and, worst, **arbitrary prose can be APPENDED into the audited span**, including a trailing "CORRECTION: the above paragraph is WRONG" retraction, with every tooth still reading the correct text above it — closed by a span-length CEILING (11 lines today, ceiling 12) beside the existing floor of 5. VERIFIER phase: a fourth — banning the two literal spellings of the stale citation is dodged by `main.ts's line 1574`, the exact bypass **rb-62 had already measured and fixed one PR earlier**; T1 now bans the drifted DIGITS, closing every spelling at once. That a sibling's published lesson was not carried across is itself the finding worth recording. **Boy Scout: NONE, deliberately.** Six sites in this same file (`:37, :465, :593, :623, :785, :975`) cite `ui/overlayA11y.ts:111` for the deferred-focus `setTimeout(0)` macrotask; `:111` is `previous.uninstall();` and the real macrotask is `:134-136` today. Six non-contiguous hunks against this loop's <=3-hunk cap, and partial de-drift would leave the file internally contradictory, so it is deferred WHOLE — the same call ADR-0163 D8 and ADR-0164 D7 made for this class rather than exceed the cap. Registered as a residual, not left in prose. **Also disclosed, not fixed:** `main.a11yFocus.test.ts:316`/`:570-571` assert `MenuView.show()` "does not call `openOverlayA11y` at all today", which m23-s6 falsified (`ui/menuView.ts:135`) — `ARCHITECTURE.md:2125` records that same dead premise as an accepted residual assigned to S10, so correcting it here would pre-empt S10 and falsify that record. The `:1627-1641` drift at `ui/dialogueView.ts:17` and `ui/dialogueView.test.ts:243/:290/:374` is NOT re-disclosed here: rb-62 already registered it as R-rb-62-R-rb62-DIALOGUEVIEWCITE. The rb-36 record at `:2200` is left untouched — like the rb-18 record it references, it is a true historical statement of what rb-36 found and flagged, and this record is what closes it. ADR next-free = 0243.

**rb-65** (residual R-rb-40-CASCADE), **ADR-0243** at the supervisor-reserved number. The deletion cascade and the data export each bind the count `purge_export_bundles` returns (rb-40 left both callers discarding it) and emit ONE terminal `mr_log` line: `account_deletion_reaper` → `{"evt":"account_deletion_cascade","subject":"<hex>","export_bundle":N}` after the PRV1-6e stamp; `request_data_export` → `{"evt":"data_export","subject":"<hex>","purged":N,"written":M}` after the rb-48 self-arm (whose plain `.insert(` can still abort the transaction — the arm is the last WRITE, the emission the last STATEMENT; ADR-0235 D2's rollback argument generalised to a thirteen-step transaction). Pure private builders `cascade_fields(subject, export_chunks)` (accounts.rs, `format!`) and `export_fields(subject, purged, written)` (privacy.rs, JSON micro-builder + a 3-line `json_usize_into`), the privacy evt via `stringify!` because the module's gate-enforced hygiene admits no quote byte (privacy.rs measured at zero quote bytes beyond the `#[path]` attribute, zero `log::`, exactly one `#[cfg` before and after) — this deliberately REVERSES ADR-0235's third rejection reason ("the first emission into the scanner-inert file"): the reducer that calls the helper owns the emission, and the hygiene contract is a set of scanner facts the new code satisfies. `subject` is a log FIELD never a label; the Anonymize-policy `account` row keeps the same key permanently while Loki self-purges at 30d, so the log is the shorter-lived copy (reducer-security-auditor PERMIT-WITH-CONDITIONS C1–C8, all verified on the artifact); semantics are PRE-COMMIT and AT-LEAST-ONCE, the SSOT for "was X erased" is `account.terminal_at_ms`. Extension contract (D4): one unquoted key per erase/anonymize/purge HELPER noun, appended in cascade order — the eleven remaining helpers are `-> ()` by ADR-0228 D1 in eleven modules outside `touches:`, so per-step counts are the ledger's **X8 `DEFER: -> backlog`** (with the file list), not prose. `/simplify` cut a planned `"steps":13` constant (a scan-re-derived runtime constant is the ADR-0224-retired pattern) and two meta-tests (a roster count, a hygiene restatement). **Teeth:** 11 `rb65`/`rb65p` tests — 7 RED-arm source scans measured RED BY NAME on the pre-fix tree (`memory/projects/gates/rb-65.red-before.md`: crate 866 run / 856 passed / 10 failed = the seven + the three re-frozen consumers, each on its predicted clause) and 4 GREEN-arm behavioural tests that call the builders; re-freezes of `rb24_frozen_reaper_body` (two independent transcriptions, guard prefix intact), rb40 `[emit/count-in-file]` 1→2 repaid per body + arithmetic, rb48 `[E4/arm-last]` lengthened with a `}` left anchor. **Artifact red-team (scratch worktree, measured):** 42/42 tester-register mutants killed (4 by the compiler), then FOUR CI-clean survivors of its own — a prefix `const`-guarded early `return Ok(())` smuggled through the regenerated freeze (CRITICAL: the whole cascade no-ops), a depth-0 `?` exit inside the pinned region on both sides, the PRV1-11 fail-loud arm rewritten to `return Ok(())` (a short export reporting success), and a function-pointer emission channel (`let emit: fn(&str,&str) = crate::observability::mr_log;`) invisible to every paren-anchored census — closed in a tester round 2 (whole-body exit census 3 returns / 1 `Err` / 2 `Ok` / first at the guard — all three shipped returns sit at depth 1, the red-team's depth-0 claim was wrong — plus a total `?` ban on the reaper, a depth-0 `?` census + `return Err(`==1 / `return Ok(`==0 in the export reducer's nested region, and a bare-identifier `mr_log` census equal to the qualified count in both files), re-measured 6/6 killed + the two gate-scope escapes (`macro_rules! stringify` shadow, `#[cfg(test)]` on the whole reducer) now killed INSIDE the X2 gate by two existing tests added to its filter. One residual class stays ungated on purpose: a `panic!()` prefix exit (fail-loud, transaction aborts; ADR-0243 honest-limit 7, reviewer-checklist not clause per ADR-0224). Verifier: no test weakened (asserts 644→704 / 389→437, zero removed), 5 own mutants killed, X1–X5 re-executed green, FAIL on one ADR sentence (the exit census mis-stated as depth-0/one) — fixed, re-verdict PASS. desync-guard PASS. **Ledger:** X1–X5, X7 by `mr-gates check`; X6 MANUAL (the by-name RED capture); X8 DEFER → backlog. **Residuals:** `R-rb-65-ADR0230-PRV120` (ADR-0230's "emits no log line" evidence now false at :124-125 and :149-155; distinct from R-rb-40-ADR0230), `R-rb-65-RUNBOOK-LOGCAVEAT` (runbook §9 names rows and backups but not the operational log; G24-pinned, outside touches); `R-rb-40-DASH` stays open. Five memory cards written (fn-pointer emission channel, depth-0 `?` vs return census, regenerated-freeze prefix exit, nested return Ok-vs-Err, squashed views delete whitespace inside literals). touches-delta: ADR-0243, `docs/adr/DIGEST.md`, `docs/knowledge/**` (11 line-stamp pages), this file; `schema.rs` declared, unmodified. Boy-scout: none beyond the change's own doc strands. ADR next-free = 0244.

**rb-66** (residual R-rb-40-DASH), no new ADR — no number reserved, and no new dependency or production pattern is introduced. **ops/observability only — zero game code, zero reducers, zero schema, zero client change.** rb-40 (ADR-0235) made `complete_guest_claim` emit `{"evt":"guest_claim_export_purge","guest":"<hex>","chunks":N}` as its terminal `mr_log` line, but nothing consumed it. Three artifacts shipped: `ops/observability/rules/recording.rules.yml` gains one `mr-meta` rule `mr:guest_claim_export_purge:rate5m` = `sum(rate(mr_log_events_total{evt="guest_claim_export_purge"}[5m])) or vector(0)`, appended after `mr:heartbeat:rate5m` (never above line 69 — six inbound client-side citations target `:69`), with a numbered `(1)-(4)` provenance comment in the `mr-scheduler` house style. `ops/observability/grafana/dashboards/monster-realm.json` gains panel `id: 15` at `gridPos {h:6,w:24,x:0,y:37}` — edge-touches panel 14's `y:31..37` box, which G13b's strict `boxesOverlap` permits. `ops/observability/checks/stack-config-checks.mjs` gains `checkEventHasQueriedConsumer`, appended at EOF (every inbound line citation targets ≤570, so appending drifts none). It is the INVERSE closure of `checkQueriedSeriesAreDefined`: that predicate says nothing queries an undefined series; this one says a defined event reaches a human-visible consumer. **The no-alert call is a DECISION, not a deferral:** no failure event to alert on (ADR-0235 makes the line terminal, so it exists only on committed transaction); a spike threshold would be invented rather than derived; an absence switch cries wolf every quiet night; the alert an operator would actually want needs `chunks` (a log BODY field, never a label — D12/OBS-36 cardinality, OBS-35 for identity), so it is a Loki query, not Prometheus. The five-reason argument lives in numbered `(4)` block above the rule in `rules/recording.rules.yml`. **Teeth:** 17 EARS-derived tests in `stack-config-checks.test.mjs` (section 19), three REDs captured in order (import-time; `no recording rule selects evt="guest_claim_export_purge"`; `recorded into nothing` — two arms fail independently). Then EXECUTED 16 attacks against the shipped predicate, **8 of them green end to end through `node evals/observability-stack-config.eval.mjs`**; all now regression teeth in `stack-config-checks.redteam.test.mjs` (RT-14..RT-24, measured 8/8 biting against the pre-hardening module). The **verifier then found two more mutants that survived that hardening**, both closed and landed as RT-25/RT-26 (2/2 biting): a `type: "news"` panel passed the two-name `row`/`text` deny-list — real Grafana display-only types (`news`, `dashlist`, `canvas`) carry a `targets` array and never execute it, so the deny-list is now an allow-list that fails CLOSED on an unknown type; and `{type:"prometheus", uid:"mr-loki"}` passed the type-only datasource check — Grafana routes on the `uid` while `type` is advisory metadata a hand-edit can leave stale, and `mr-loki` is a really-provisioned datasource, so a declared uid must now be the provisioned Prometheus one. Measured bypasses closed: one-line `expr:` key under alert's `labels:`/`annotations:` or `isPaused: true` forged a consumer while panel was deleted outright — closed by scoping alert exprs to a live rule's `model:` block. `expr:` key nested under recording rule's `labels:` impersonated rule body while real body recorded `vector(0)` — closed by requiring `expr:` at rule's own key indent. Verbatim `- record:` copy inside YAML block scalar parsed as real rule while Prometheus recorded nothing — closed by block-scalar tracking plus flat-list sibling rule. `hide` compared with `=== true` while Grafana disables on truthiness, so `hide: 1` and `hide: "true"` passed; string-form or `{uid:"-100"}` datasource read as "inherit, therefore Prometheus" — both closed. Zero-area (`h:0`/`w:0`) panel, `row`/`text` panel, and duplicate-`refId` target that Grafana drops all counted as consumers — closed. `#` opens PromQL comment too, so `vector(0) # mr:guest_claim_export_purge:rate5m` satisfied identifier scan — closed by quote-aware `#` cut before extraction. **Honest bounds, stated as bounds:** (a) nothing in `just ci` proves Alloy actually mints `mr_log_events_total{evt="guest_claim_export_purge"}` — needs booted stack (`MR_OBS_STACK=1`) and real guest claim; the `or vector(0)` keeps wrong inference from rendering broken rather than false. (b) Text gate cannot evaluate PromQL: six measured semantic-neutering shapes are all textually legal, so RT-24 pins the purge rule's body as DERIVED from heartbeat sibling instead. (c) Consumer scan does not recurse into collapsed row's `panel.panels[]`, so reorganising dashboard into rows REDs this gate — measured, and the safe direction. (d) Two residuals disclosed: `R-rb-66-NODEFLOOR` (G9k's `NODE_TEST_PASS_FLOOR` still 243 against an actual 271; the eval outside `touches:`) and `R-rb-66-PANELOVERLAP` (no gate enforces general dashboard `gridPos` non-overlap or panel-id uniqueness outside G13b's single id-14 scope; panel 15's placement hand-verified). **Supersession sentence:** the m20b paragraph's "18 pure text predicates" is now 19. **Gates:** 146 tests across the two checks suites (17 EARS teeth + 13 RT teeth), `observability-stack-config` eval PASS, full `just ci` exit 0. No ADR consumed — no number reserved and no new dependency or production pattern introduced; the rationale lives in the rule's provenance comment (rb-60/rb-61/rb-62 precedent). ADR next-free = 0244.
