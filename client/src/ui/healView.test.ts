...
describe('HealView — overlay a11y wiring on the render edge (m23-s3)', () => {
  ...
  it('S3-healView-CLOSE-RESTORE BITES: hide() strips role, aria-modal AND aria-label from the root and hands focus back to the pre-overlay element', async () => {
    // hide() is healView's PRODUCTION close (main.ts:1382, :364) — render(null) is unreachable for
...
// tests lean on (the list rebuild, the display flip) is pinned here rather than assumed. These
// tests pass on master TODAY and must keep passing after the S3 wiring lands.
// ---------------------------------------------------------------------------
