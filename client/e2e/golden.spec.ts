...
const snap = (p: Page): Promise<Snap> =>
  p.evaluate(() => {
    ...
    return {
      identity: g.identity, stepMs: g.stepMs, map: g.map, ownEntityId: g.ownEntityId,
      ownAuthTile: g.ownAuthTile, ownPredictedTile: g.ownPredictedTile,
      presenceCount: g.presenceCount, characters: g.characters,
    };
  });
...
test.describe.serial('M5a — two-window POC golden flows', () => {
  let browser: Browser;
  ...
  test.beforeAll(async () => {
    ...
    await a.waitForFunction(() => (window as unknown as { __game: () => Snap }).__game().presenceCount === 2, null, { timeout: 30_000 });
    await b.waitForFunction(() => (window as unknown as { __game: () => Snap }).__game().presenceCount === 2, null, { timeout: 30_000 });
  });
  ...
  test('A→B movement syncs and prediction converges (predicted == authoritative)', async () => {
    ...
    const target: Tile = { x: (ga.ownAuthTile as Tile).x + o.x, y: (ga.ownAuthTile as Tile).y + o.y };
    await a.evaluate((d) => (window as unknown as { __game: () => { step: (x: string) => void } }).__game().step(d), dir as string);
    // A: authoritative advances to target AND predicted matches it (4-step reconcile).
    await a.waitForFunction(
      (t: Tile) => {
        const g = (window as unknown as { __game: () => Snap }).__game();
        return !!g.ownAuthTile && g.ownAuthTile.x === t.x && g.ownAuthTile.y === t.y
          && !!g.ownPredictedTile && g.ownPredictedTile.x === t.x && g.ownPredictedTile.y === t.y;
      },
      target,
      { timeout: 15_000 },
    );
    ...
    await b.waitForFunction(
      (args: { id: string; t: Tile }) => {
        const c = (window as unknown as { __game: () => Snap }).__game().characters.find((x) => x.entityId === args.id);
  ...
  test('B→A movement syncs (symmetric)', async () => {
    ...
    const o = OFFSET[dir as string];
    const target: Tile = { x: (gb.ownAuthTile as Tile).x + o.x, y: (gb.ownAuthTile as Tile).y + o.y };
    await b.evaluate((d) => (window as unknown as { __game: () => { step: (x: string) => void } }).__game().step(d), dir as string);
    await a.waitForFunction(
      (args: { id: string; t: Tile }) => {
        const c = (window as unknown as { __game: () => Snap }).__game().characters.find((x) => x.entityId === args.id);
        return !!c && c.tileX === args.t.x && c.tileY === args.t.y;
  ...
  test('wall bump leaves predicted == authoritative (the canonical no-desync net)', async () => {
    ...
    await a.evaluate((d) => (window as unknown as { __game: () => { step: (x: string) => void } }).__game().step(d), wall as string);
