...
pub(crate) fn sync_content_inner(ctx: &ReducerContext) -> Result<(), String> {
    validate_enum_rosters()?;
    // The roster gate runs ABOVE the version gate on purpose: its predicate is
    // data-independent (it compares compiled-in constants against derive
...
/// A minimal typespace builder that resolves each type inline instead of
/// interning it behind a reference handle.
///
/// The `SpacetimeType` derive hands its `AlgebraicType` to a builder; the real
/// module-ABI builder assigns a `Ref` per named type, which would hide a
/// fieldless enum's variant list behind an indirection. This one simply runs
...
/// The variant names `T`'s `SpacetimeType` derive metadata declares, in
/// declaration order.
///
/// This is the ORACLE the hand-maintained rosters are measured against: it is
/// generated by the derive macro from the enum declaration itself, so it grows
/// automatically when a variant is added and cannot drift the way a literal
/// array does. `enum_name` is diagnostic only — it names the type in the error
/// so a CI failure points at the roster that needs growing.
///
/// # Errors
///
/// Returns `Err` when `T`'s derive metadata is not a named-variant sum: a
/// primitive, a product, an empty sum, or a sum with an unnamed variant. The
/// failure is a returned error rather than a panic on purpose — the caller runs
/// inside a reducer, where an unwrap would abort the transaction with no
/// operator-visible diagnostic at all.
fn reflected_variant_names<T: spacetimedb::SpacetimeType>(
    enum_name: &str,
) -> Result<Vec<String>, String> {
    let reflected = <T as spacetimedb::SpacetimeType>::make_type(&mut EnumRosterTypespace);
    if let spacetimedb::sats::AlgebraicType::Sum(sum) = &reflected {
        let names: Vec<String> = sum
            .variants
            .iter()
            .filter_map(|variant| variant.name.as_deref().map(String::from))
            .collect();
        if !sum.variants.is_empty() && names.len() == sum.variants.len() {
            return Ok(names);
        }
    }
    Err(format!(
        "{enum_name}: derive metadata is not a named-variant sum, so its \
         declared variants cannot be reflected. The roster totality check only \
         applies to a fieldless enum carrying a derived SpacetimeType."
    ))
}

/// Check that `roster` — a hand-maintained array of every value of `T` — really
/// does list every declared variant of `T`, exactly once.
///
/// Totality is proven by two clauses together, because either one alone is
/// forgeable: the roster's length must equal the reflected variant count, AND
/// the roster's entries must be pairwise distinct. Length alone accepts a
/// roster that repeats one variant and omits another; distinctness alone
/// accepts a short roster.
///
/// # Contract
///
/// `T` must be a FIELDLESS enum whose `PartialEq` is DERIVED. The distinctness
/// clause compares VALUES, and for a payload-carrying enum two distinct values
/// need not be two distinct variants — a roster of two `Poison(1)`/`Poison(2)`
/// values would pass distinctness while covering one variant. A hand-written
/// `PartialEq` that ignores a field reintroduces the same hole.
///
/// # Errors
///
/// Returns `Err` when the roster's length differs from the reflected variant
/// count, or when two roster entries compare equal. Every message names
/// `enum_name` verbatim and states BOTH counts, so the operator reading a CI
/// failure can tell which roster drifted and in which direction.
fn check_roster_is_total<T: spacetimedb::SpacetimeType + PartialEq + core::fmt::Debug>(
    enum_name: &str,
    roster: &[T],
) -> Result<(), String> {
    let reflected = reflected_variant_names::<T>(enum_name)?;
    ...
    for (index, entry) in roster.iter().enumerate() {
        for later in &roster[index + 1..] {
            if entry == later {
                return Err(format!(
                    "{enum_name}: the hand-maintained roster lists {entry:?} more \
                     than once, so its {} entries cover fewer than the {} \
                     declared variants {:?}. Replace the duplicate in the SOURCE \
                     roster with the variant it is hiding.",
                    roster.len(),
                    reflected.len(),
                    reflected,
                ));
...
/// Re-derive every hand-maintained enum roster from its enum's derive metadata
/// and reject the first one that is not total.
///
/// `game_core::validate_a11y_tokens` builds its REQUIRED token-key set by
/// walking `STATUS_KIND_ALL` and `Affinity::ALL`. Adding a variant to either
/// enum is a compile error in the exhaustive match arms, so the author fixes
/// those and the crate builds — but the fixed-size roster does not grow, the
/// new key never enters the required set, and content validation returns `Ok`
/// with an unlabelled variant shipped. These two calls are what turn that
/// silent hole into a failing build.
///
/// # Errors
///
/// Returns the first roster that does not cover its enum, with both counts and
/// the reflected variant names in the message.
fn validate_enum_rosters() -> Result<(), String> {
    check_roster_is_total::<game_core::StatusKind>(
        "StatusKind",
        &game_core::content::STATUS_KIND_ALL,
    )?;
    check_roster_is_total::<game_core::Affinity>("Affinity", &game_core::Affinity::ALL)
}
