...
/// ALL whitespace removed. Grown HERE: `economy_tests.rs` has no squash helper
/// and ADR-0003's copy-never-import rule forbids a third shared stripper.
...
/// **E1 (roster closure)** — `economy.rs` publishes EXACTLY 2 reducers, and this
/// is the module that moves CURRENCY and INVENTORY. T0 (measured, pristine tree):
/// a plain bare twin appended above the test-module tail SURVIVED 915/915 — this
/// file had NO cover. GREEN AT HEAD and after: ZERO production edits, an ADR-0224
...
///
/// Designated register rows (`memory/projects/gates/rb-81.mutants.py`): M1c the
/// plain twin · M2c the conditional-compilation-hidden twin · M3 the wire-name
/// twin with the real function demoted. Rationale: ADR-0251 D1-D4.
#[test]
fn rb81_economy_reducer_roster_is_closed() {
    ...
    let n_char_quote = ECONOMY_SOURCE.matches(char_quote.as_str()).count()
        + ECONOMY_SOURCE.matches(char_quote_escaped.as_str()).count();
    assert_eq!(
        n_char_quote, 0,
    ...
    assert!(
        ...
        "[rb81/substrate] P4 FAIL: the comment region(s) starting at line(s) {odd_regions:?} \
         of `economy.rs` carry an ODD number of double quotes. An odd region opens a phantom \
    ...
    assert!(
        order_a == order_b,
        "[rb81/substrate] P5 FAIL: the two stripper orders disagree on `economy.rs` (lengths \
    ...
    let qs = q.as_str();
    let path_attr = ["#", "[path=", qs, "economy_tests.rs", qs, "]"].concat();
    let cfg_test = ["#", "[cfg(test)]"].concat();
    let mod_decl = ["mod", "economy", "_tests;"].concat();
    let allow_attr = ["#", "[allow(unused_imports)]"].concat();
    let tail = format!("{cfg_test}{path_attr}{allow_attr}{mod_decl}");
    ...
    let want = std::collections::BTreeSet::from([["b", "uy"].concat(), ["se", "ll"].concat()]);
    ...
    assert_eq!(
        ...
        "[rb81/attr-any] FAIL: `economy.rs` carries {n_any} reducer attribute(s) but only \
         ...
         reads is a different, possibly ungated, one. This file carries the register's M3 row."
    );
    ...
    assert_eq!(
        ...
        "[rb81/include-ban] FAIL: the substring `include` appears {n_inc} time(s) on the \
         stripped view of `economy.rs` (any spelling — macro or identifier) and must appear \
    ...
    // --- 10 [rb81/attr-raw] -------------------------------------------------
    let n_raw_attrs = rb81_attr_openers(ECONOMY_SOURCE);
    assert_eq!(
        n_raw_attrs, 5,
        "[rb81/attr-raw] FAIL: the RAW text of `economy.rs` carries {n_raw_attrs} attribute \
         ...
         here spells an attribute opener, so this number and the partition total move \
         together, and a comment that merely QUOTES an attribute reds this clause alone — a \
         deliberate re-review, not a false alarm. An opener is a hash whose next \
         NON-WHITESPACE byte is the bracket, so a hash spaced away from its bracket still \
         counts and this census does not lean on the formatter gate. The only clause reading \
         NO stripper output, so it still bites if the strippers are ever fooled; adding an \
