...
// ===========================================================================
// rb-81 (R-rb-47-ROSTER-PVP) — ADR-0251: economy.rs's REDUCER ROSTER IS CLOSED.
//
...
/// ALL whitespace removed. Grown HERE because `economy_tests.rs` has no squash
/// helper of its own and ADR-0003's copy-never-import rule applies: a
/// `#[cfg(test)]` child module cannot reach a sibling test module's items, and a
/// THIRD shared stripper is what that rule exists to prevent.
fn rb81_squash_economy(src: &str) -> String {
    src.chars().filter(|c| !c.is_whitespace()).collect()
}
...
/// clause bans. `strip_rust_strings_economy` (:976) has no raw lexer, so ONE of
/// these leaves the byte-walker half a literal out of phase for the whole file.
...
fn rb81_reducer_names(squashed: &str) -> Vec<String> {
    ...
    while let Some(rel) = squashed[from..].find(attr.as_str()) {
        ...
        let tail = rest.strip_prefix(public_fn.as_str()).unwrap_or_else(|| {
            ...
            panic!(
                "[rb81/roster-parse] E1 FAIL: a bare reducer attribute in `economy.rs` is not \
        ...
        assert!(
            ...
            "[rb81/roster-parse] E1 FAIL: a bare reducer attribute in `economy.rs` is followed \
             by a declaration this walk cannot name (empty identifier, or no argument list). An \
...
/// **E1 (roster closure)** — `economy.rs` declares EXACTLY the two reducers this
/// slice reasoned about, over a view whose substrate is pinned FIRST.
///
/// WHY. rb-47 closed `trading.rs` and rb-80 closed the raising cluster;
/// `economy.rs` had NOTHING. T0 measured the gap on the pristine tree: a plain
...
/// enumerates — `rb46_gated_reducer_census_battle_and_economy` included — so a
/// third reducer here is constrained by nothing at all, and this is the module
/// that moves CURRENCY and INVENTORY.
///
...
///
/// sole kills (`memory/projects/gates/rb-81.mutants.py`): M1c the plain twin ·
/// M2c a conditional-compilation-hidden twin · M3 the wire-name twin with the
/// real function demoted — this file carries that row, so both its
/// `[rb81/attr-any]` and its lint-silencing-attribute arms are measured here.
#[test]
fn rb81_economy_reducer_roster_is_closed() {
    // --- 1 [rb81/substrate] -------------------------------------------------
    let quote = char::from(0x22u8);
    let open_block = ["/", "*"].concat();
    let close_block = ["*", "/"].concat();
    let n_open = ECONOMY_SOURCE.matches(open_block.as_str()).count();
    let n_close = ECONOMY_SOURCE.matches(close_block.as_str()).count();
    assert_eq!(
        (n_open, n_close),
        (0, 0),
        "[rb81/substrate] P1 FAIL: `economy.rs` carries {n_open} block-comment opener(s) and \
         {n_close} closer(s); both must be ZERO. `strip_rust_comments_economy` (:936) reads an \
         UNPAIRED marker inside a string literal as a real comment and blanks code to the next \
         closer — which is how a twin reducer disappears from every clause below."
    );

    let tick = char::from(0x27u8);
    let char_quote: String = [tick, quote, tick].iter().collect();
    let n_char_quote = ECONOMY_SOURCE.matches(char_quote.as_str()).count();
    assert_eq!(
        n_char_quote, 0,
        "[rb81/substrate] P2 FAIL: `economy.rs` spells the double-quote CHAR literal \
    ...
    let n_raw_open = rb81_raw_string_openers(ECONOMY_SOURCE);
    assert_eq!(
        n_raw_open, 0,
        "[rb81/substrate] P3 FAIL: `economy.rs` opens {n_raw_open} raw string(s) and must open \
         ZERO. `strip_rust_strings_economy` (:976) has no raw-string lexer, so a raw literal \
         holding a bare quote leaves the walker half a literal out of phase for the rest of \
         the file."
    );

    let odd_regions = rb81_odd_quote_comment_regions(ECONOMY_SOURCE);
    assert!(
        odd_regions.is_empty(),
        "[rb81/substrate] P4 FAIL: the comment region(s) starting at line(s) {odd_regions:?} of \
         `economy.rs` carry an ODD number of double quotes. An odd region opens a phantom \
         string literal that swallows real code under a strings-first pass — the \
         desynchronisation P5 then cannot see, because both orders are equally wrong."
    );

    let stripped = strip_rust_strings_economy(&strip_rust_comments_economy(ECONOMY_SOURCE));
    let order_a = rb81_squash_economy(&stripped);
    let order_b = rb81_squash_economy(&strip_rust_comments_economy(&strip_rust_strings_economy(
        ECONOMY_SOURCE,
    )));
    let diverge = order_a
        .char_indices()
    ...
    assert!(
        ...
        "[rb81/substrate] P5 FAIL: comments-then-strings and strings-then-comments disagree on \
         `economy.rs` (lengths {len_a}/{len_b}, first divergence at byte {diverge:?}). The \
         orders can only differ when a comment marker lives inside a string literal or a quote \
         lives unbalanced inside a comment — i.e. when the scanned view is no longer the file."
    );
    let file = order_a;

    // --- 2 [rb81/tail] ------------------------------------------------------
    let comments_only = rb81_squash_economy(&strip_rust_comments_economy(ECONOMY_SOURCE));
    ...
    let tail = [
        ["#", "[cfg(test)]"].concat(),
        path_attr,
        ["#", "[allow(unused_imports)]"].concat(),
        ["mod", "economy", "_tests;"].concat(),
    ]
    .concat();
    ...
    assert!(
        ...
        "[rb81/tail] FAIL: the comment-stripped, strings-INTACT view of `economy.rs` must END \
         ...
         ships. The lint-silencing attribute is part of the pin because it is one of the five \
         the partition accounts for."
    );

    // --- 3 [rb81/cfg-roster] ------------------------------------------------
    let cfgs = rb81_cfg_predicates(comments_only.as_str());
    let want_cfgs = [["#", "[cfg(test)]"].concat()];
    assert_eq!(
        cfgs, want_cfgs,
        "[rb81/cfg-roster] FAIL: `economy.rs` carries the conditional-compilation predicates \
         {cfgs:?} and must carry exactly {want_cfgs:?}. The roster below is a claim about the \
         DEFAULT build: a predicate widened to an always-true form publishes a reducer that \
         build compiles while the text roster is unchanged, and a predicate ADDED to a \
         production item is how a gate is present in review and absent in the shipped wasm."
    );
    ...
    assert_eq!(
        n_bare, 2,
        "[rb81/roster] FAIL (count): `economy.rs` carries {n_bare} bare reducer attribute(s) \
         and must carry exactly 2; the names this walk resolved are {got:?}. Reported BEFORE \
         the SET comparison because it is not implied by it: a twin the walk resolves to an \
         existing name leaves the SET equal while the file publishes one more entry point."
    );
    let want: std::collections::BTreeSet<String> = [["b", "uy"].concat(), ["se", "ll"].concat()]
        .into_iter()
        .collect();
    ...
    assert!(
        ...
        "[rb81/roster] FAIL (set): the reducers `economy.rs` publishes are {got:?}; the roster \
         ...
         UNEXPECTED NAME IS THE DANGEROUS DIRECTION — this module is the single wallet-mutation \
         surface, so a third reducer is an unreviewed path to `spend_currency` / \
         ...
         in this file (they all scope to the two names they mention). A MISSING name means a \
         reducer was renamed or removed and every pin scoped to it is now vacuous. Classify a \
         new reducer DELIBERATELY and add it here AND to the censuses that must fence it; never \
    ...
    assert_eq!(
        n_any, n_bare,
        "[rb81/attr-any] FAIL: `economy.rs` carries {n_any} reducer attribute(s) but only \
         ...
         reads is a different, possibly ungated, function — the register's M3 row, measured in \
         this file."
    );
    ...
    assert_eq!(
        n_path_token, n_bare,
        "[rb81/attr-path] FAIL: `economy.rs` spells the path-qualified reducer token \
    ...
    let kinds: [(&str, String, usize); 4] = [
        ("reducer", attr_any.clone(), 2),
        ("cfg", ["#", "[cfg("].concat(), 1),
        ("path", ["#", "[path"].concat(), 1),
        ("allow", ["#", "[allow("].concat(), 1),
    ];
    ...
    for (i, (label, _, want_n)) in kinds.iter().enumerate() {
        ...
        assert_eq!(
            ...
            "[rb81/attr-partition] FAIL ({label}): found/expected per kind: {census}. THE \
             ...
             reducer one at all, a conditional attribute expanding to it. The lint-silencing \
             kind is counted for the same reason: the measured wire-name twin demotes the real \
             function and silences the dead-code warning that follows. THIS IS A SECURITY \
             CENSUS: add the attribute deliberately, then bump this kind's pin and the total in \
             the SAME edit; never relax a number alone."
        );
    }
    let attr_open = ["#", "["].concat();
    ...
    assert_eq!(
        n_all, want_total,
        "[rb81/attr-partition] FAIL (total): `economy.rs` carries {n_all} attribute opener(s) \
    ...
    assert_eq!(
        n_mod, 1,
        "[rb81/mod-census] FAIL: `economy.rs` declares {n_mod} module(s) and must declare \
         ...
         shadowing module re-exporting the reducer macro under another name defeats the path \
         clause too."
    );
    ...
    assert_eq!(
        n_inc, 0,
        "[rb81/include-ban] FAIL: `economy.rs` mentions the source-inlining macro family \
         {n_inc} time(s) and must mention it ZERO. It splices another file's tokens into THIS \
         module at compile time, so the spliced reducer is published while no view built from \
         this file's text can ever see it."
    );
    let mac = ["macro", "_rules"].concat();
    let n_mac = stripped.matches(mac.as_str()).count();
    assert_eq!(
        n_mac, 0,
        "[rb81/macro-ban] FAIL: `economy.rs` defines {n_mac} declarative macro(s) and must \
         define ZERO. A macro assembling the attribute from fragments publishes an entry point \
         every literal needle here counts as zero. A macro defined in ANOTHER module and \
         invoked here is outside this file's window: R-rb-81-CROSSFILEMACRO."
    );

    // --- 10 [rb81/attr-raw] -------------------------------------------------
    let n_raw_attrs = ECONOMY_SOURCE.matches(attr_open.as_str()).count();
    assert_eq!(
        n_raw_attrs, 5,
        "[rb81/attr-raw] FAIL: the RAW text of `economy.rs` carries {n_raw_attrs} attribute \
         opener(s) and must carry 5. UNLIKE ITS THREE SIBLINGS THE DELTA IS ZERO: no comment \
         here spells an attribute opener, so this number and the partition total are the same \
         number and move together. The only clause reading NO stripper output, so it still \
         bites if the strippers are ever fooled — which also means a comment that merely QUOTES \
         an attribute reds this clause alone. That is a deliberate re-review, not a false \
         alarm: bump both numbers in the same edit."
    );
}
